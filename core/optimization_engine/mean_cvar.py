    ...
    def optimize(
        ...
        try:
            ...
            if constraints_obj.min_return is not None:
                ...
                min_return_daily = (1.0 + constraints_obj.min_return) ** (
                    1.0 / self.periods_per_year
                ) - 1.0
                constraints_list.append(expected_return_daily >= min_return_daily)
                ...
                if last_status == "infeasible":
                    ...
                    if constraints_obj.min_return is not None:
                        ...
                        relaxed_constraints = [
                            ...
                            if not (
                                isinstance(c, cp.constraints.Inequality)
                                and hasattr(c, "args")
                                and len(c.args) > 0
                                and str(c.args[0]).find("expected_return") >= 0
                            )
                        ]
                        ...
                        if optimization_mode == "cvar_cap":
                            relaxed_min_problem = cp.Problem(
                                cp.Minimize(cvar), relaxed_constraints
                            )
                            relaxed_min_problem.solve(solver=cp.SCS, verbose=False)
                            if relaxed_min_problem.status in [
                                "optimal",
                                "optimal_inaccurate",
                            ]:
                                relaxed_min_cvar_value = float(
                                    relaxed_min_problem.value
                                )
                                relaxed_cvar_cap = (
                                    float(cvar_cap_relax) * relaxed_min_cvar_value
                                )
                                relaxed_cap_constraints = list(relaxed_constraints) + [
                                    cvar <= relaxed_cvar_cap
                                ]
                                relaxed_problem = cp.Problem(
                                    cp.Maximize(expected_return_daily),
                                    relaxed_cap_constraints,
                                )
                            else:
                                relaxed_problem = cp.Problem(
                                    cp.Maximize(
                                        expected_return_daily - risk_aversion * cvar
                                    ),
                                    relaxed_constraints,
                                )
                        else:
            ...
            var_value = np.percentile(portfolio_returns, (1.0 - confidence_level) * 100)
            tail_returns = portfolio_returns[portfolio_returns <= var_value]
            cvar_value = (
                float(tail_returns.mean()) if len(tail_returns) > 0 else var_value
            )
            ...
            return OptimizationResult(
                ...
                metadata={
                    ...
                    "cvar_cap_relax": (
                        float(cvar_cap_relax)
                        if optimization_mode == "cvar_cap"
                        else None
                    ),
                    "min_cvar_lp_value": (
                        float(min_cvar_value)
                        if optimization_mode == "cvar_cap"
                        and min_cvar_value is not None
                        else None
                    ),
                    "cvar_cap": (
                        float(cvar_cap)
                        if optimization_mode == "cvar_cap" and cvar_cap is not None
                        else None
                    ),
