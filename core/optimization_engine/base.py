...
class BaseOptimizer(ABC):
    ...
    def _validate_weights(
        ...
        if abs(weight_sum - 1.0) > 0.01:
            logger.warning(f"Weights sum to {weight_sum:.4f}, expected 1.0")
            return False
    ...
    def _calculate_portfolio_metrics(
        ...
        # Portfolio variance
        portfolio_variance = float(weights.T @ self._cov_matrix @ weights)
    ...
    def _estimate_covariance_matrix(
        ...
        if method == "shrink":
            ...
            shrunk_daily = (1.0 - alpha) * sample_cov_daily.values + alpha * diag_target
            return pd.DataFrame(
