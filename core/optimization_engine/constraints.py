...
class OptimizationConstraints:
    ...
    def set_turnover_constraint(
        ...
        if min_trade_size is not None and (min_trade_size < 0 or min_trade_size > 1):
    ...
    def set_cardinality_constraint(
        ...
        Returns:
            Self for method chaining
        """
        if min_assets is not None and (min_assets < 1 or min_assets > self.n_assets):
            raise ValueError(f"min_assets must be between 1 and {self.n_assets}")

        if max_assets is not None and (max_assets < 1 or max_assets > self.n_assets):
            raise ValueError(f"max_assets must be between 1 and {self.n_assets}")
    ...
    def get_weight_bounds_array(
        ...
        if self.max_cash_weight is not None:
            ...
            for cash_idx in cash_indices:
                max_bounds[cash_idx] = min(max_bounds[cash_idx], self.max_cash_weight)
        ...
        for group_name, group_data in self.group_constraints.items():
            ...
            def make_group_constraint(group_tickers, max_w):
                def constraint(weights: np.ndarray) -> float:
                    group_indices = [self.tickers.index(t) for t in group_tickers]
