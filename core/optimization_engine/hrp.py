...
class HRPOptimizer(BaseOptimizer):
    ...
    def optimize(
        ...
        try:
            ...
            distance_matrix = np.sqrt(np.clip(0.5 * (1.0 - corr_matrix), 0.0, None))
            ...
            cov_ordered = cov_df.iloc[ordered_indices, ordered_indices].values
            ...
            if constraints_obj.long_only and (weights < 0).any():
                logger.warning("HRP produced negative weights, clipping to zero")
