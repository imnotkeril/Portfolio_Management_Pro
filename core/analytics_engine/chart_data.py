...
def get_rolling_metric_data(
    ...
    if metric_name.lower() == "sharpe":
        ...
        metric_series = (
            portfolio_returns.rolling(window).std() * np.sqrt(TRADING_DAYS_PER_YEAR)
        ).fillna(0)
...
def get_rolling_sharpe_data(
    ...
    portfolio_sharpe = ((rolling_mean - daily_rf) / rolling_std) * np.sqrt(
    ...
    if benchmark_returns is not None and not benchmark_returns.empty:
        ...
        benchmark_sharpe = ((bench_mean - daily_rf) / bench_std) * np.sqrt(
            TRADING_DAYS_PER_YEAR
        )
...
def get_rolling_sortino_data(
    ...
    def rolling_sortino(returns, window_size):
        ...
        for i in range(window_size - 1, len(returns)):
            ...
            if downside > 0:
                result.iloc[i] = (mean_excess / downside) * np.sqrt(
                    TRADING_DAYS_PER_YEAR
                )
            else:
                result.iloc[i] = 0.0
...
def get_rolling_beta_alpha_data(
    ...
    rolling_alpha = portfolio_mean - (
        risk_free_rate + rolling_beta * (benchmark_mean - risk_free_rate)
    )
...
def get_underwater_plot_data(
    ...
    if not portfolio_values.empty:
        ...
        result["max_drawdown"] = {"date": max_dd_idx, "value": float(underwater.min())}
    ...
    if benchmark_values is not None and not benchmark_values.empty:
        ...
        aligned_benchmark = (
            benchmark_values.reindex(portfolio_values.index, method="ffill")
            .bfill()
            .ffill()
        )
...
def get_drawdown_periods_data(
    ...
    """
    if portfolio_values.empty:
        return {"cumulative_returns": pd.Series(dtype=float), "drawdown_zones": []}
    ...
    if in_drawdown.any():
        ...
        for group_id, group_data in in_drawdown.groupby(groups):
            ...
            zone = {"start": period_dates[0], "end": period_dates[-1], "depth": min_dd}
            drawdown_zones.append(zone)

    return {"cumulative_returns": cumulative_returns, "drawdown_zones": drawdown_zones}
...
def get_drawdown_recovery_data(
    ...
    for i, dd in enumerate(top_drawdowns, 1):
        # Get peak value
        peak_value = (
            cum_returns.loc[dd["start_date"]]
            if dd["start_date"] in cum_returns.index
            else None
        )
        trough_value = (
            cum_returns.loc[dd["bottom_date"]]
            if dd["bottom_date"] in cum_returns.index
            else None
        )
        recovery_value = (
            cum_returns.loc[dd["recovery_date"]]
            if dd["recovery_date"] and dd["recovery_date"] in cum_returns.index
            else None
        )
        ...
        timeline_data = {
            ...
            "recovery_value": (
                float(recovery_value) if recovery_value is not None else None
            ),
        }
...
def get_comparison_stats_data(
    ...
    avg_win = (
        float((winning_days["portfolio"] - winning_days["benchmark"]).mean())
    ...
    avg_loss = (
        float((losing_days["portfolio"] - losing_days["benchmark"]).mean())
...
def get_yearly_returns_data(
    ...
    if benchmark_returns is not None and not benchmark_returns.empty:
        ...
        yearly_benchmark = (
            (1 + bench_df["Benchmark"]).groupby(bench_df["Year"]).prod() - 1
        ) * 100
        # Align indices
            ...
            if start_date:
                ...
                try:
                    ...
                    if len(period_returns) > 0:
                        portfolio_periods[display_name] = float(
        ...
        for display_name, key in period_map.items():
            ...
            if start_date:
                ...
                try:
                    ...
                    if len(period_returns) > 0:
                        benchmark_periods[display_name] = float(
                            (1 + period_returns).prod() - 1
                        )
                    else:
...
def get_three_month_rolling_periods_data(
    ...
    window = 63
    portfolio_rolling = portfolio_returns.rolling(window).apply(
    ...
    def filter_non_overlapping(df_sorted, top_n):
        ...
        while len(selected) < top_n and len(df_sorted) > 0:
            ...
            mask = df_sorted.apply(
                lambda row: not (
                    pd.to_datetime(row["Start"]) <= current_end
                    and pd.to_datetime(row["End"]) >= current_start
                ),
                axis=1,
            )
        ...
        if selected:
            ...
            return pd.DataFrame(
                columns=["Start", "End", "Portfolio", "Benchmark", "Difference"]
            )
...
def get_monthly_active_returns_data(
    ...
    monthly_active = (
        aligned.groupby([aligned["year"], aligned["month"]])["active"].sum() * 100
    )
...
def get_win_rate_statistics_data(
    ...
    weekly_stats = (
        calculate_win_rate_stats(weekly_returns, "weekly")
    ...
    monthly_stats = (
        calculate_win_rate_stats(monthly_returns, "monthly")
        if not monthly_returns.empty
        else {}
    )

    # Quarterly win rate
    quarterly_returns = portfolio_returns.resample("Q").apply(
        lambda x: (1 + x).prod() - 1
    )
    quarterly_stats = (
        calculate_win_rate_stats(quarterly_returns, "quarterly")
    ...
    yearly_stats = (
        calculate_win_rate_stats(yearly_returns, "yearly")
    ...
    portfolio_returns.resample("ME").apply(lambda x: (1 + x).prod() - 1)

    # For each day, get returns from last 12 months (252 trading days)
    for i in range(len(portfolio_returns)):
        portfolio_returns.index[i]
        ...
        if len(window_returns) >= 20:  # At least 20 days to have meaningful data
            ...
            monthly_in_window = window_returns.resample("ME").apply(
                lambda x: (1 + x).prod() - 1
            )
    ...
    if benchmark_returns is not None and not benchmark_returns.empty:
        ...
        bench_weekly_returns = benchmark_returns.resample("W").apply(
            lambda x: (1 + x).prod() - 1
        )
        bench_weekly = (
            calculate_win_rate_stats(bench_weekly_returns, "weekly")
            if not bench_weekly_returns.empty
            else {}
        )
        bench_monthly_returns = benchmark_returns.resample("ME").apply(
            lambda x: (1 + x).prod() - 1
        )
        bench_monthly = (
            calculate_win_rate_stats(bench_monthly_returns, "monthly")
            if not bench_monthly_returns.empty
            else {}
        )
        bench_quarterly_returns = benchmark_returns.resample("Q").apply(
            lambda x: (1 + x).prod() - 1
        )
        bench_quarterly = (
            calculate_win_rate_stats(bench_quarterly_returns, "quarterly")
            if not bench_quarterly_returns.empty
            else {}
        )
        bench_yearly_returns = benchmark_returns.resample("Y").apply(
            lambda x: (1 + x).prod() - 1
        )
        bench_yearly = (
            calculate_win_rate_stats(bench_yearly_returns, "yearly")
            if not bench_yearly_returns.empty
            else {}
        )

        bench_stats = {
            "win_rate_daily": (
                bench_daily.get("win_rate_daily", 0) if bench_daily else 0
            ),
            "win_rate_weekly": (
                bench_weekly.get("win_rate_weekly", 0) if bench_weekly else 0
            ),
            "win_rate_monthly": (
                bench_monthly.get("win_rate_monthly", 0) if bench_monthly else 0
            ),
            "win_rate_quarterly": (
                bench_quarterly.get("win_rate_quarterly", 0) if bench_quarterly else 0
            ),
            "win_rate_yearly": (
                bench_yearly.get("win_rate_yearly", 0) if bench_yearly else 0
            ),
            "avg_win_daily": bench_daily.get("avg_win_daily", 0) if bench_daily else 0,
            "avg_loss_daily": (
                bench_daily.get("avg_loss_daily", 0) if bench_daily else 0
            ),
            "avg_win_monthly": (
                bench_monthly.get("avg_win_monthly", 0) if bench_monthly else 0
            ),
            "avg_loss_monthly": (
                bench_monthly.get("avg_loss_monthly", 0) if bench_monthly else 0
            ),
            ...
            "best_monthly": (
                bench_monthly.get("best_monthly", 0) if bench_monthly else 0
            ),
            "worst_monthly": (
                bench_monthly.get("worst_monthly", 0) if bench_monthly else 0
            ),
        }
    ...
    stats = {
        "portfolio": {
            "win_days_pct": (
                daily_stats.get("win_rate_daily", 0) * 100
                if daily_stats and daily_stats.get("win_rate_daily") is not None
                else 0
            ),
            "win_weeks_pct": (
                weekly_stats.get("win_rate_weekly", 0) * 100
                if weekly_stats and weekly_stats.get("win_rate_weekly") is not None
                else 0
            ),
            "win_months_pct": (
                monthly_stats.get("win_rate_monthly", 0) * 100
                if monthly_stats and monthly_stats.get("win_rate_monthly") is not None
                else 0
            ),
            "win_quarters_pct": (
                quarterly_stats.get("win_rate_quarterly", 0) * 100
                if quarterly_stats
                and quarterly_stats.get("win_rate_quarterly") is not None
                else 0
            ),
            "win_years_pct": (
                yearly_stats.get("win_rate_yearly", 0) * 100
                if yearly_stats and yearly_stats.get("win_rate_yearly") is not None
                else 0
            ),
            "avg_up_day": (
                daily_stats.get("avg_win_daily", 0) * 100
                if daily_stats and daily_stats.get("avg_win_daily") is not None
                else 0
            ),
            "avg_down_day": (
                daily_stats.get("avg_loss_daily", 0) * 100
                if daily_stats and daily_stats.get("avg_loss_daily") is not None
                else 0
            ),
            "avg_up_month": (
                monthly_stats.get("avg_win_monthly", 0) * 100
                if monthly_stats and monthly_stats.get("avg_win_monthly") is not None
                else 0
            ),
            "avg_down_month": (
                monthly_stats.get("avg_loss_monthly", 0) * 100
                if monthly_stats and monthly_stats.get("avg_loss_monthly") is not None
                else 0
            ),
            "best_day": (
                daily_stats.get("best_daily", 0) * 100
                if daily_stats and daily_stats.get("best_daily") is not None
                else 0
            ),
            "worst_day": (
                daily_stats.get("worst_daily", 0) * 100
                if daily_stats and daily_stats.get("worst_daily") is not None
                else 0
            ),
            "best_month": (
                monthly_stats.get("best_monthly", 0) * 100
                if monthly_stats and monthly_stats.get("best_monthly") is not None
                else 0
            ),
            "worst_month": (
                monthly_stats.get("worst_monthly", 0) * 100
                if monthly_stats and monthly_stats.get("worst_monthly") is not None
                else 0
            ),
        },
...
def get_outlier_analysis_data(
    ...
    if len(normal_wins) > 0 and len(outlier_wins) > 0:
        ...
        outlier_win_ratio = (
            avg_outlier_win / avg_normal_win if avg_normal_win != 0 else 0.0
        )
    ...
    if len(normal_losses) > 0 and len(outlier_losses) > 0:
        ...
        outlier_loss_ratio = (
            avg_outlier_loss / avg_normal_loss if avg_normal_loss != 0 else 0.0
        )
...
def get_risk_return_scatter_data(
    ...
    try:
        ...
        port_vol = (
            port_vol_dict.get("annual", 0.0)
            if isinstance(port_vol_dict, dict)
            else port_vol_dict
        )
        ...
        if benchmark_returns is not None and not benchmark_returns.empty:
            bench_return = calculate_annualized_return(
...
def get_rolling_var_data(
    ...
    try:
        ...
        portfolio_var = portfolio_returns.rolling(window=window).apply(
            lambda x: (
                calculate_var(x, confidence_level, "historical")
                if len(x) >= window
                else np.nan
            ),
            raw=False,
        ...
        if benchmark_returns is not None and not benchmark_returns.empty:
            ...
            benchmark_var = aligned_benchmark.rolling(window=window).apply(
                lambda x: (
                    calculate_var(x, confidence_level, "historical")
                    if len(x) >= window
                    else np.nan
...
def get_rolling_volatility_data(
    ...
    try:
        ...
        portfolio_vol = portfolio_returns.rolling(window=window).std() * np.sqrt(
            periods_per_year
        )
        ...
        if benchmark_returns is not None and not benchmark_returns.empty:
            ...
            benchmark_vol = aligned_benchmark.rolling(window=window).std() * np.sqrt(
                periods_per_year
            )
...
def get_rolling_alpha_data(
    ...
    try:
        ...
        def calculate_alpha(port_window, bench_window):
            ...
            alpha = port_mean_annual - (
                risk_free_rate + beta * (bench_mean_annual - risk_free_rate)
            )
...
def get_rolling_active_return_data(
    ...
    try:
        ...
        rolling_port_return = (
            portfolio_returns.rolling(window=window).mean() * periods_per_year
        )
        rolling_bench_return = (
            aligned_benchmark.rolling(window=window).mean() * periods_per_year
        )
        ...
        pct_positive = (
            (positive_periods / total_periods * 100) if total_periods > 0 else 0
        )
        ...
        for i in range(window, len(portfolio_returns) + 1):
            ...
            if bull_mask_window.sum() >= 2:
                rolling_bull_beta.iloc[i - 1] = calc_beta(
                    port_window[bull_mask_window], bench_window[bull_mask_window]
                )

            if bear_mask_window.sum() >= 2:
                rolling_bear_beta.iloc[i - 1] = calc_beta(
                    port_window[bear_mask_window], bench_window[bear_mask_window]
                )
...
def get_asset_impact_on_return_data(
    ...
    try:
        ...
        if start_date is not None:
            ...
            if isinstance(filtered_data.index, pd.DatetimeIndex):
                ...
                try:
                    ...
                    logger.warning(f"Could not filter by start_date {start_date}")
        ...
        if end_date is not None:
            if isinstance(filtered_data.index, pd.DatetimeIndex):
                ...
                try:
                    ...
                    logger.warning(f"Could not filter by end_date {end_date}")
        ...
        contributions = {}
        for ticker in weights.keys():
            weighted_return = weights[ticker] * asset_returns.get(ticker, 0.0)
            contributions[ticker] = float(weighted_return * 100)

        # Sort by contribution
        sorted_tickers = sorted(
            contributions.keys(), key=lambda t: contributions[t], reverse=True
        )

        return {
            "tickers": sorted_tickers,
            "contributions": [contributions[t] for t in sorted_tickers],
            "returns": [asset_returns.get(t, 0.0) * 100 for t in sorted_tickers],
            "weights": [weights[t] * 100 for t in sorted_tickers],
        }

    except Exception as e:
        logger.warning(f"Error calculating asset impact on return: {e}")
        import traceback
...
def get_asset_impact_on_risk_data(
    ...
    try:
        ...
        weights_array = np.array(
            [weights.get(ticker, 0.0) for ticker in aligned_returns.columns]
        )
        portfolio_variance = np.dot(
            weights_array, np.dot(cov_matrix.values, weights_array)
        )
        ...
        # Sort by contribution
        sorted_tickers = sorted(
            risk_contributions.keys(), key=lambda t: risk_contributions[t], reverse=True
        )
...
def get_risk_vs_weight_comparison_data(
    ...
    if return_data and return_data.get("tickers"):
        return_dict = dict(zip(return_data["tickers"], return_data["contributions"]))
...
def get_diversification_coefficient_data(
    ...
    try:
        ...
        weighted_sum_vols = sum(
            weights[ticker] * individual_vols.get(ticker, 0.0)
            for ticker in weights.keys()
        )
...
def get_correlation_with_benchmark_data(
    ...
    try:
        ...
        tickers = [
            pos.ticker
            for pos in positions
            if pos.ticker in asset_returns_aligned.columns
        ]
        ...
        for ticker in tickers:
            ...
            aligned = pd.DataFrame(
                {"asset": asset_ret, "benchmark": benchmark_aligned}
            ).dropna()
...
def get_asset_price_dynamics_data(
    ...
    try:
        ...
        if selected_tickers is None:
            selected_tickers = [
                pos.ticker for pos in positions if pos.ticker in price_data.columns
            ]
        ...
        return {
            ...
            "dates": (
                price_series[list(price_series.keys())[0]].index
                if price_series
                else None
            ),
        }
...
def get_rolling_correlations_data(
    ...
    try:
        ...
        tickers = [pos.ticker for pos in positions if pos.ticker in returns_df.columns]
...
def get_average_correlation_to_portfolio_data(
    ...
    try:
        ...
        return {
            "tickers": sorted_tickers,
            "avg_correlations": [avg_correlations[t] for t in sorted_tickers],
            "diversification_scores": [
        ...
        logger.warning(f"Error calculating average correlation to portfolio: {e}")
        return None
    ...
    try:
        ...
        for ticker in selected_tickers:
            ...
            for i in range(window, len(asset_ret) + 1):
                ...
                # Align windows
                aligned = pd.DataFrame(
                    {"asset": window_asset, "benchmark": window_bench}
                ).dropna()
        ...
        if portfolio_returns is not None and not portfolio_returns.empty:
            ...
            if len(aligned_port) >= window:
                ...
                for i in range(window, len(aligned_port) + 1):
                    ...
                    aligned = pd.DataFrame(
                        {"portfolio": window_port, "benchmark": window_bench}
                    ).dropna()
    ...
    try:
        ...
        for other_ticker in other_tickers:
            ...
            aligned_other = pd.DataFrame(
                {"asset": asset_returns, "other": other_returns}
            ).dropna()
