...
def calculate_outlier_analysis(
    ...
    try:
        ...
        outlier_win_ratio = (
            ...
            if len(outlier_wins) > 0
            and len(normal_wins) > 0
            and normal_wins.mean() != 0
            else 1.0
        )

        outlier_loss_ratio = (
            abs(outlier_losses.mean() / normal_losses.mean())
            if len(outlier_losses) > 0
            and len(normal_losses) > 0
            and normal_losses.mean() != 0
            else 1.0
        )
        ...
        return {
            ...
            "outlier_pct": float(
                (len(outlier_wins) + len(outlier_losses)) / len(returns)
            ),
        }
