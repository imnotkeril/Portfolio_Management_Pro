...
def _interpret_forecast_comparison(
    ...
    for row in comparison_data:
        try:
            ...
            final_val_str = (
                row.get("Forecast Value", "$0.00").replace("$", "").replace(",", "")
            )
    ...
    if len(final_values) > 1:
        ...
        if cv < 0.05:
            parts.append(
                f"High agreement: Forecasts are very similar (CV: {cv:.1%}) - Methods agree on direction and magnitude"
            )
        elif cv < 0.15:
            parts.append(
                f"Moderate agreement: Some variation in forecasts (CV: {cv:.1%}) - Methods generally agree"
            )
        else:
            parts.append(
                f"⚠ Low agreement: Significant variation in forecasts (CV: {cv:.1%}) - Methods disagree, consider ensemble"
            )
    ...
    if len(positive_changes) > len(negative_changes):
        parts.append(
            f"Most methods ({len(positive_changes)}/{num_methods}) predict price increase"
        )
    elif len(negative_changes) > len(positive_changes):
        parts.append(
            f"Most methods ({len(negative_changes)}/{num_methods}) predict price decrease"
        )
    else:
        parts.append(
            f"Mixed predictions: {len(positive_changes)} positive, {len(negative_changes)} negative"
        )
    ...
    if final_values:
        ...
        best_method = (
            method_names[best_idx] if best_idx < len(method_names) else "Unknown"
        )
        worst_method = (
            method_names[worst_idx] if worst_idx < len(method_names) else "Unknown"
        )

        parts.append(
            f"Highest forecast: {best_method} (${final_values[best_idx]:,.2f}, {changes[best_idx]:+.2f}%)"
        )
        parts.append(
            f"Lowest forecast: {worst_method} (${final_values[worst_idx]:,.2f}, {changes[worst_idx]:+.2f}%)"
        )
    ...
    if mape_values:
        ...
        parts.append(
            f"Best accuracy (lowest MAPE): {best_mape[0]} ({best_mape[1]:.2f}%)"
        )
...
def _interpret_individual_forecast(
    ...
    if historical_values is not None and len(historical_values) > 0:
        current_value = (
            float(historical_values[-1]) if np.isfinite(historical_values[-1]) else 0.0
        )
        if current_value > 0:
            parts.append(
                f"Current value: ${current_value:,.2f} → Forecast: ${final_value:,.2f} ({change_pct:+.2f}% change)"
            )
    ...
    if change_pct > 5:
        ...
        parts.append(
            f"Moderate upward forecast: Expected increase of {change_pct:.2f}%"
        )
    elif change_pct < -5:
        parts.append(
            f"⚠ Strong downward forecast: Expected decrease of {abs(change_pct):.2f}%"
        )
    elif change_pct < 0:
        parts.append(
            f"Moderate downward forecast: Expected decrease of {abs(change_pct):.2f}%"
        )
    else:
    ...
    if ci and isinstance(ci, dict):
        ...
        if upper_key and lower_key:
            try:
                upper = (
                    float(ci[upper_key])
                    if isinstance(ci[upper_key], (int, float))
                    else 0.0
                )
                lower = (
                    float(ci[lower_key])
                    if isinstance(ci[lower_key], (int, float))
                    else 0.0
                )
                if upper > 0 and lower > 0:
                    ci_range = (
                        ((upper - lower) / final_value) * 100 if final_value > 0 else 0
                    )
                    if ci_range > 20:
                        parts.append(
                            f"Wide confidence interval ({ci_range:.1f}% range) - High forecast uncertainty"
                        )
                    elif ci_range > 10:
                        parts.append(
                            f"Moderate confidence interval ({ci_range:.1f}% range) - Some forecast uncertainty"
                        )
                    else:
                        parts.append(
                            f"Narrow confidence interval ({ci_range:.1f}% range) - Relatively confident forecast"
                        )
            except (ValueError, TypeError):
                pass
    ...
    if validation_metrics and isinstance(validation_metrics, dict):
        ...
        try:
            ...
            if np.isfinite(mape):
                if mape < 5:
                    parts.append(
                        f"Excellent accuracy: MAPE {mape:.2f}% - Very reliable forecast"
                    )
                elif mape < 10:
                    parts.append(f"Good accuracy: MAPE {mape:.2f}% - Reliable forecast")
                elif mape < 20:
                    parts.append(
                        f"Moderate accuracy: MAPE {mape:.2f}% - Some forecast uncertainty"
                    )
                else:
                    parts.append(
                        f"⚠ Low accuracy: MAPE {mape:.2f}% - High forecast uncertainty"
                    )
        except (ValueError, TypeError):
            pass
...
def _interpret_residuals_analysis(residuals: np.ndarray, method_name: str) -> str:
    ...
    if abs(mean_residual) > std_residual * 0.5:
        if mean_residual > 0:
            parts.append(
                f"⚠ Positive bias: Model tends to overestimate (mean: {mean_residual:.4f})"
            )
        else:
            parts.append(
                f"⚠ Negative bias: Model tends to underestimate (mean: {mean_residual:.4f})"
            )
    else:
        parts.append(
            "No significant bias: Mean residual close to zero - Model is unbiased"
        )
    ...
    if len(residuals) > 10:
        ...
        if first_half_std > 0:
            ...
            if variance_ratio > 1.5:
                parts.append(
                    "⚠ Increasing variance: Residuals become more variable over time - Possible heteroscedasticity"
                )
            elif variance_ratio < 0.67:
                parts.append(
                    "Decreasing variance: Residuals become less variable over time"
                )
            else:
                parts.append(
                    "Constant variance: Residuals show stable variance - Good model fit"
                )
    ...
    if len(residuals) > 5:
        ...
        if abs(trend_coef) > std_residual * 0.1:
            if trend_coef > 0:
                parts.append(
                    "⚠ Upward trend in residuals: Model errors increasing over time"
                )
            else:
                parts.append(
                    "⚠ Downward trend in residuals: Model errors decreasing over time"
                )
        else:
            parts.append(
                "No trend: Residuals are randomly distributed - Good model fit"
            )
...
def _interpret_forecast_quality(
    ...
    safe_parse_metric(quality_df["RMSE"], "$")
    ...
    if len(valid_dir) > 0:
        ...
        parts.append(
            f"Best direction accuracy: {best_dir_method} ({best_dir_val:.1f}%)"
        )
    ...
    if len(valid_mape) > 0:
        ...
        if avg_mape < 5:
            parts.append(
                f"Excellent overall accuracy: Average MAPE {avg_mape:.2f}% - All methods perform well"
            )
        elif avg_mape < 10:
            parts.append(
                f"Good overall accuracy: Average MAPE {avg_mape:.2f}% - Methods are reliable"
            )
        elif avg_mape < 20:
            parts.append(
                f"Moderate overall accuracy: Average MAPE {avg_mape:.2f}% - Some forecast uncertainty"
            )
        else:
            parts.append(
                f"⚠ Low overall accuracy: Average MAPE {avg_mape:.2f}% - High forecast uncertainty"
            )
    ...
    if len(valid_dir) > 0:
        ...
        if avg_dir > 70:
            parts.append(
                f"High direction accuracy: Average {avg_dir:.1f}% - Methods correctly predict price direction"
            )
        elif avg_dir > 55:
            parts.append(
                f"Moderate direction accuracy: Average {avg_dir:.1f}% - Methods somewhat reliable for direction"
            )
        else:
            parts.append(
                f"⚠ Low direction accuracy: Average {avg_dir:.1f}% - Methods struggle with direction prediction"
            )
    ...
    if len(valid_r2) > 0:
        ...
        if avg_r2 > 0.8:
            parts.append(
                f"Excellent fit: Average R² {avg_r2:.3f} - Models explain most variance"
            )
        elif avg_r2 > 0.5:
            parts.append(
                f"Good fit: Average R² {avg_r2:.3f} - Models explain significant variance"
            )
        elif avg_r2 > 0:
            parts.append(
                f"Moderate fit: Average R² {avg_r2:.3f} - Models explain some variance"
            )
        else:
            parts.append(
                f"⚠ Poor fit: Average R² {avg_r2:.3f} - Models explain little variance"
            )
...
def _interpret_method_ranking(ranked_df: pd.DataFrame) -> str:
    ...
    if len(ranked_df) > 0:
        ...
        try:
            ...
            if not np.isfinite(top_score):
                top_score = 0.5
            parts.append(
                f"Best overall method: {top_method} (Score: {top_score*100:.1f}%)"
            )
        except (ValueError, TypeError):
            pass
    ...
    if len(scores) > 1:
        ...
        if score_std < 0.1:
            parts.append(
                "Methods perform similarly: Small score differences - All methods are competitive"
            )
        elif score_std < 0.2:
            parts.append(
                "Moderate performance differences: Some methods clearly better than others"
            )
        else:
            parts.append(
                "Large performance differences: Clear winner and losers - Consider using best method or ensemble"
            )
    ...
    if len(ranked_df) > 1:
        ...
        try:
            ...
            if not np.isfinite(bottom_score):
                bottom_score = 0.5
            parts.append(
                f"Worst performing method: {bottom_method} (Score: {bottom_score*100:.1f}%)"
            )
        except (ValueError, TypeError):
            pass
...
def _interpret_model_info(model_info: dict, method_name: str) -> str:
    ...
    if aic is not None:
        try:
            ...
            if np.isfinite(aic_val):
                parts.append(
                    f"AIC: {aic_val:.2f} - Lower is better (model fit vs complexity trade-off)"
                )
        except (ValueError, TypeError):
            pass
    ...
    if bic is not None:
        try:
            ...
            if np.isfinite(bic_val):
                parts.append(
                    f"BIC: {bic_val:.2f} - Lower is better (stronger penalty for complexity than AIC)"
                )
        except (ValueError, TypeError):
            pass
        ...
        try:
            ...
            if np.isfinite(time_val) and time_val >= 0:
                if time_val < 1:
                    ...
                    parts.append(
                        f"Long training time: {time_val:.2f}s - Computationally intensive model"
                    )
        except (ValueError, TypeError):
            pass
    ...
    if "auto" in model_info:
        ...
        if auto_val:
            parts.append(
                "Auto parameters: Enabled - Model automatically selected optimal parameters"
            )
...
def render_forecasting_page() -> None:
    ...
    if forecast_type == "Single Asset":
        ...
        if not all_tickers:
            st.warning(
                "No tickers found in portfolios. Please create a portfolio first."
            )
            return
    ...
    with method_categories[0]:
        ...
        if use_arima:
            ...
            if model_desc:
                with st.expander(
                    f"ℹ️ {model_desc.get('short', 'ARIMA')}", expanded=False
                ):
            ...
            with st.expander("ARIMA Parameters", expanded=False):
                ...
                if not auto_arima:
                    _render_label_with_help(
                        "p (AR order)", PARAMETER_HELP.get("arima", {}).get("p", "")
                    )
                    arima_p = st.slider("", 0, 5, 1, key="arima_p")

                    _render_label_with_help(
                        "d (Differencing)", PARAMETER_HELP.get("arima", {}).get("d", "")
                    )
                    arima_d = st.slider("", 0, 2, 1, key="arima_d")

                    _render_label_with_help(
                        "q (MA order)", PARAMETER_HELP.get("arima", {}).get("q", "")
                    )
        ...
        if use_garch:
            ...
            if model_desc:
                with st.expander(
                    f"ℹ️ {model_desc.get('short', 'GARCH')}", expanded=False
                ):
                    st.markdown(model_desc.get("detailed", ""))

            with st.expander("GARCH Parameters", expanded=False):
                _render_label_with_help(
                    "GARCH p (lag order)", PARAMETER_HELP.get("garch", {}).get("p", "")
                )
                garch_p = st.slider("", 1, 3, 1, key="garch_p")

                _render_label_with_help(
                    "GARCH q (lag order)", PARAMETER_HELP.get("garch", {}).get("q", "")
                )
        ...
        if use_arima_garch:
            ...
            if model_desc:
                with st.expander(
                    f"ℹ️ {model_desc.get('short', 'ARIMA-GARCH')}", expanded=False
                ):
                    st.markdown(model_desc.get("detailed", ""))
            ...
            with st.expander("ARIMA-GARCH Parameters", expanded=False):
                ...
                if not arima_garch_auto:
                    ...
                    arima_garch_garch_p = st.slider(
                        "", 1, 3, 1, key="arima_garch_garch_p"
                    )
                    ...
                    arima_garch_garch_q = st.slider(
                        "", 1, 3, 1, key="arima_garch_garch_q"
                    )
                    ...
                    arima_garch_garch_p = st.slider(
                        "", 1, 3, 1, key="arima_garch_garch_p_auto"
                    )
                    ...
                    arima_garch_garch_q = st.slider(
                        "", 1, 3, 1, key="arima_garch_garch_q_auto"
                    )
    ...
    with method_categories[1]:
        ...
        if use_xgboost:
            ...
            if model_desc:
                with st.expander(
                    f"ℹ️ {model_desc.get('short', 'XGBoost')}", expanded=False
                ):
                    st.markdown(model_desc.get("detailed", ""))
            ...
            with st.expander("XGBoost Parameters", expanded=False):
                ...
                _render_label_with_help(
                    "Max Depth", PARAMETER_HELP.get("xgboost", {}).get("max_depth", "")
                )
                ...
                xgb_learning_rate = st.slider(
                    "", 0.01, 0.3, 0.1, key="xgb_learning_rate"
                )
                ...
                use_technical_features = st.checkbox(
                    ...
                    True,
                    key="xgb_technical",
                    help=PARAMETER_HELP.get("xgboost", {}).get(
        ...
        if use_random_forest:
            ...
            if model_desc:
                with st.expander(
                    f"ℹ️ {model_desc.get('short', 'Random Forest')}", expanded=False
                ):
                    st.markdown(model_desc.get("detailed", ""))
        ...
        if use_svm:
            ...
            if model_desc:
                with st.expander(
                    f"ℹ️ {model_desc.get('short', 'SVM/SVR')}", expanded=False
                ):
                    st.markdown(model_desc.get("detailed", ""))

            with st.expander("SVM Parameters", expanded=False):
                _render_label_with_help(
                    "C (Regularization)", PARAMETER_HELP.get("svm", {}).get("C", "")
                )
                ...
                _render_label_with_help(
                    "Kernel", PARAMETER_HELP.get("svm", {}).get("kernel", "")
                )
    ...
    with method_categories[2]:
        ...
        if use_ssa_maemd_tcn:
            ...
            if model_desc:
                with st.expander(
                    f"ℹ️ {model_desc.get('short', 'SSA-MAEMD-TCN')}", expanded=False
                ):
                    st.markdown(model_desc.get("detailed", ""))
        ...
        if use_lstm:
            ...
            if model_desc:
                with st.expander(
                    f"ℹ️ {model_desc.get('short', 'LSTM')}", expanded=False
                ):
                    st.markdown(model_desc.get("detailed", ""))
    ...
    with method_categories[3]:
        ...
        if use_prophet:
            ...
            if model_desc:
                with st.expander(
                    f"ℹ️ {model_desc.get('short', 'Prophet')}", expanded=False
                ):
                    st.markdown(model_desc.get("detailed", ""))

            with st.expander("Prophet Parameters", expanded=False):
                _render_label_with_help(
                    "Growth Model", PARAMETER_HELP.get("prophet", {}).get("growth", "")
                )
    ...
    with method_categories[4]:
        ...
        if use_ensemble:
            ...
            if model_desc:
                with st.expander(
                    f"ℹ️ {model_desc.get('short', 'Ensemble')}", expanded=False
                ):
                    st.markdown(model_desc.get("detailed", ""))
            ...
            with col1:
                ...
                ensemble_arima = st.checkbox(
                    "Include ARIMA", True, key="ensemble_arima"
                )
                ensemble_garch = st.checkbox(
                    "Include GARCH", False, key="ensemble_garch"
                )
                ensemble_arima_garch = st.checkbox(
                    "Include ARIMA-GARCH", False, key="ensemble_arima_garch"
                )
                ensemble_prophet = st.checkbox(
                    "Include Prophet", True, key="ensemble_prophet"
                )

            with col2:
                st.markdown("**ML & Deep Learning Models:**")
                ensemble_xgboost = st.checkbox(
                    "Include XGBoost", True, key="ensemble_xgboost"
                )
                ensemble_random_forest = st.checkbox(
                    "Include Random Forest", False, key="ensemble_random_forest"
                )
    ...
    if (
        "saved_forecast_results" in st.session_state
        and st.session_state["saved_forecast_results"]
        and not run_forecast_button
    ):
        ...
        try:
            for idx, method in enumerate(selected_methods):
                ...
                if st.session_state.get("forecast_run_id") != current_run_id:
                    st.warning(
                        "Previous forecast run was cancelled. Starting new run..."
                    )
                    break
                ...
                try:
                    ...
                    if st.session_state.get("forecast_run_id") != current_run_id:
                        st.warning(
                            f"Forecast run cancelled after {method} completed. Results discarded."
                        )
                        break
            ...
            if st.session_state.get("forecast_run_id") != current_run_id:
                st.warning(
                    "Forecast run was cancelled. Skipping ensemble creation and clearing results."
                )
            ...
            if (
                use_ensemble
                and st.session_state.get("forecast_run_id") == current_run_id
            ):
                try:
                    ensemble_methods = []
                    ...
                    if len(ensemble_methods) >= 2:
                        ensemble_forecasts = {m: forecasts[m] for m in ensemble_methods}
            ...
            if use_out_of_sample:
                ...
                if forecasts:
                    for method, forecast_data in forecasts.items():
                        if (
                            forecast_data
                            and isinstance(forecast_data, dict)
                            and forecast_data.get("success", False)
                        ):
                            if (
                                "forecast_dates" in forecast_data
                                and forecast_data["forecast_dates"]
                            ):
            ...
            st.session_state["saved_results_training_start"] = (
                training_start if use_out_of_sample else None
            )
            ...
            st.session_state["saved_results_ticker"] = (
                selected_ticker if forecast_type == "Single Asset" else None
            )
            st.session_state["saved_results_portfolio_id"] = (
                selected_portfolio_id if forecast_type == "Portfolio" else None
            )
...
def _display_forecast_results(
    ...
    try:
        if ticker:
            ...
            chart_start_date = (
                training_start if (training_start and use_out_of_sample) else start_date
            )
            ...
            try:
                ...
                if tickers:
                    ...
                    chart_start_date = (
                        training_start
                        if (training_start and use_out_of_sample)
                        else start_date
                    )
                    ...
                    if not hist_price_data.empty:
                        ...
                        if (
                            "Ticker" in hist_price_data.columns
                            and "Adjusted_Close" in hist_price_data.columns
                        ):
                            if "Date" in hist_price_data.columns:
                                hist_price_data["Date"] = pd.to_datetime(
                                    hist_price_data["Date"], errors="coerce"
                                )
                                hist_price_data["Date"] = hist_price_data[
                                    "Date"
                                ].dt.tz_localize(None)
                        ...
                        forecasting_service = ForecastingService()
                        portfolio_prices = (
                            forecasting_service.calculate_portfolio_prices(
    ...
    with results_tabs[0]:
        ...
        if historical_dates is not None and historical_values is not None:
            ...
            if (
                "successful_forecasts" in st.session_state
                and st.session_state["successful_forecasts"]
            ):
            ...
            if (
                "saved_results_forecast_end" in st.session_state
                and st.session_state["saved_results_forecast_end"]
            ):
                forecast_end_ts = pd.Timestamp(
                    st.session_state["saved_results_forecast_end"]
                )
            elif forecast_end:
            ...
            st.plotly_chart(
                fig,
                use_container_width=True,
                key=f"forecast_comparison_chart_with_history_{chart_suffix}",
            )
        else:
            ...
            for method_name, forecast_data in successful_forecasts.items():
                ...
                try:
                    ...
                    if not np.any(valid_mask):
                        logger.warning(
                            f"Method {method_name}: no valid forecast_values"
                        )
                        continue
                    ...
                    if len(forecast_dates) == 0 or len(forecast_values) == 0:
                        logger.warning(
                            f"Method {method_name}: empty forecast data after filtering"
                        )
                        continue
            ...
            st.plotly_chart(
                fig,
                use_container_width=True,
                key=f"forecast_comparison_chart_no_history_{chart_suffix}",
            )
        ...
        for method_name, forecast_data in successful_forecasts.items():
            ...
            final_value = safe_get_value(
                forecast_data, "final_value", 0, lambda x: f"${x:.2f}"
            )
            change_pct = safe_get_value(
                forecast_data, "change_pct", 0, lambda x: f"{x:.2f}%"
            )
        ...
        interpretation = _interpret_forecast_comparison(
            successful_forecasts, comparison_data, historical_values
        )
        if interpretation:
    ...
    with results_tabs[1]:
        ...
        if historical_dates is not None and historical_values is not None:
            ...
            st.plotly_chart(
                fig,
                use_container_width=True,
                key=f"forecast_individual_chart_with_history_{chart_suffix}",
            )

            # Interpretation: Individual forecast
            interpretation = _interpret_individual_forecast(
                selected_forecast, method_selector, historical_values
            )
            if interpretation:
                st.info(interpretation)
        else:
            ...
            try:
                ...
                logger.warning(
                    f"Error parsing forecast_dates for {method_selector}: {e}"
                )
                forecast_dates = pd.DatetimeIndex([])
            ...
            try:
                ...
                logger.warning(
                    f"Error parsing forecast_values for {method_selector}: {e}"
                )
                forecast_values = np.array([])
            ...
            fig.add_trace(
                go.Scatter(
                    ...
                    line=dict(
                        color=method_color, width=2
                    ),  # Solid line with method color
                )
            )

            # Add confidence intervals if available
            if (
                "confidence_intervals" in selected_forecast
                and selected_forecast["confidence_intervals"]
            ):
                ...
                if isinstance(ci, dict):
                    ...
                    if upper_key and lower_key:
                        try:
                            ...
                            # Align lengths
                            min_len = min(
                                len(forecast_dates), len(upper_95), len(lower_95)
                            )
                            if min_len > 0:
                                fig.add_trace(
                                    go.Scatter(
                            ...
                            logger.warning(
                                f"Error plotting confidence intervals for {method_selector}: {e}"
                            )
            ...
            st.plotly_chart(
                fig,
                use_container_width=True,
                key=f"forecast_individual_chart_{chart_suffix}",
            )
        ...
        if (
            "residuals" in selected_forecast
            and selected_forecast["residuals"] is not None
        ):
            ...
            logger.debug(
                f"Residuals for {method_selector}: type={type(residuals_raw)}, is None={residuals_raw is None}"
            )
                ...
                try:
                    ...
                    if isinstance(residuals_raw, list):
                        if len(residuals_raw) > 0:
                            ...
                            logger.debug(
                                f"Converted residuals from list: length={len(residuals)}"
                            )
                        else:
                            logger.warning(
                                f"Residuals list is empty for {method_selector}"
                            )
                        ...
                        residuals = np.array(residuals_raw, dtype=float)
                        logger.debug(
                            f"Converted residuals from {type(residuals_raw)}: length={len(residuals)}"
                        )
                    ...
                    if residuals is not None and len(residuals) > 0:
                        ...
                        st.plotly_chart(
                            fig_residuals,
                            use_container_width=True,
                            key=f"residuals_chart_{method_selector}",
                        )

                    # Interpretation: Residuals analysis
                    interpretation = _interpret_residuals_analysis(
                        residuals, method_selector
                    )
                    if interpretation:
                        st.info(interpretation)
                    else:
                        st.info(
                            "No valid residuals data available for this forecast method."
                        )
                except Exception as e:
                    logger.error(
                        f"Error plotting residuals for {method_selector}: {e}",
                        exc_info=True,
                    )
                    logger.error(
                        f"Residuals type: {type(residuals_raw)}, value sample: {residuals_raw[:5] if hasattr(residuals_raw, '__getitem__') and len(residuals_raw) > 0 else 'N/A'}"
                    )
                    st.warning(f"Could not plot residuals: {e}")
            else:
                logger.debug(
                    f"No residuals key found or residuals is None for {method_selector}"
                )
    ...
    with results_tabs[2]:
        ...
        if not use_out_of_sample:
            ...
            for method_name, forecast_data in forecasts_to_use.items():
                if not isinstance(forecast_data, dict):
                    logger.warning(
                        f"Method {method_name}: forecast_data is not a dict (type={type(forecast_data)})"
                    )
                    continue

                metrics = forecast_data.get("validation_metrics")
                logger.debug(
                    f"Method {method_name}: validation_metrics type={type(metrics)}, value={metrics}"
                )
                ...
                if metrics and isinstance(metrics, dict):
                    ...
                    logger.warning(
                        f"Method {method_name}: validation_metrics is missing or not a dict (type={type(metrics)})"
                    )
            ...
            if quality_data:
                ...
                interpretation = _interpret_forecast_quality(
                    quality_df, forecasts_to_use
                )
                if interpretation:
                    st.info(interpretation)
                ...
                for metric in metrics_to_plot:
                    ...
                    st.plotly_chart(
                        fig_quality,
                        use_container_width=True,
                        key=f"quality_chart_{metric}",
                    )

                # Overall ranking based on all metrics
                st.markdown("### Overall Method Ranking")
                st.markdown(
                    "Ranking based on all quality metrics (lower is better for MAPE, RMSE, MAE; higher is better for Direction Accuracy and R²):"
                )
                ...
                numeric_df["Direction_num"] = safe_convert_to_float(
                    numeric_df["Direction Accuracy"], "%"
                )
                ...
                mape_range = (
                    max_mape - min_mape
                    if max_mape != min_mape and max_mape > 0
                    else 1.0
                )
                rmse_range = (
                    max_rmse - min_rmse
                    if max_rmse != min_rmse and max_rmse > 0
                    else 1.0
                )
                mae_range = (
                    max_mae - min_mae if max_mae != min_mae and max_mae > 0 else 1.0
                )
                dir_range = (
                    max_dir - min_dir if max_dir != min_dir and max_dir > 0 else 1.0
                )
                ...
                numeric_df["score"] = numeric_df["score"].replace(
                    [np.inf, -np.inf], 0.5
                )
                ...
                for idx, (_, row) in enumerate(ranked_df.iterrows(), 1):
                    try:
                        ...
                        st.markdown(
                            f"{idx}. **{method_name}** - Overall Score: {score_pct:.1f}%"
                        )
                    except Exception as e:
    ...
    with results_tabs[3]:
        ...
        if st.session_state[detail_selector_key] in available_methods:
            current_index = available_methods.index(
                st.session_state[detail_selector_key]
            )
            ...
            with col2:
                ...
                if "bic" in model_info and model_info["bic"] is not None:
                    try:
                        ...
                        pass
                if (
                    "training_time" in model_info
                    and model_info["training_time"] is not None
                ):
                    try:
        ...
        with col2:
            ...
            try:
                final_value_detail = (
                    float(final_value_detail) if final_value_detail is not None else 0.0
                )
        ...
        with col3:
            ...
            try:
                change_pct_detail = (
                    float(change_pct_detail) if change_pct_detail is not None else 0.0
                )
        ...
        if (
            "validation_metrics" in selected_detail
            and selected_detail["validation_metrics"]
        ):
            ...
            if isinstance(metrics, dict):
                ...
                def safe_metric_display(
                    key, default=0, format_str="{:.2f}%", prefix=""
                ):
                ...
                with col4:
                    dir_display = safe_metric_display(
                        "direction_accuracy", np.nan, "{:.1f}%"
                    )
