...
def _interpret_qq_plot(
    ...
    fit_quality = (
        "excellent"
        if abs_mean_dev < 0.001
        else (
            "good"
            if abs_mean_dev < 0.005
            else "moderate" if abs_mean_dev < 0.01 else "poor"
        )
    )
    ...
    interpretation_parts.append("**Distribution Analysis:**")

    # Overall fit
    if fit_quality == "excellent":
        interpretation_parts.append(
            "✓ Distribution is very close to normal (excellent fit)"
        )
    elif fit_quality == "good":
        interpretation_parts.append("✓ Distribution is approximately normal (good fit)")
    elif fit_quality == "moderate":
        interpretation_parts.append("⚠ Distribution deviates moderately from normal")
    else:
        interpretation_parts.append("⚠ Distribution deviates significantly from normal")

    # Tail analysis
    if has_fat_tails:
        if has_left_tail and has_right_tail:
            interpretation_parts.append(
                "⚠ **Fat tails detected:** Both tails deviate from normal. This means extreme gains and losses occur more frequently than expected in a normal distribution."
            )
        elif has_left_tail:
            interpretation_parts.append(
                "⚠ **Left tail risk:** Extreme losses occur more frequently than normal distribution predicts. Higher downside risk."
            )
        elif has_right_tail:
            interpretation_parts.append(
                "✓ **Right tail benefit:** Extreme gains occur more frequently than normal. Potential for larger positive surprises."
            )
    else:
        interpretation_parts.append("✓ Tails are close to normal distribution")

    # Skewness
    if is_skewed_left:
        interpretation_parts.append(
            "⚠ **Negative skew:** Distribution is skewed left. More frequent small gains but occasional large losses."
        )
    elif is_skewed_right:
        interpretation_parts.append(
            "✓ **Positive skew:** Distribution is skewed right. More frequent small losses but occasional large gains."
        )
    else:
        interpretation_parts.append("✓ Distribution is approximately symmetric")

    # Practical implications
    interpretation_parts.append("\n**Practical Implications:**")
    if has_fat_tails or fit_quality in ["moderate", "poor"]:
        interpretation_parts.append(
            "- VaR and CVaR estimates may underestimate tail risk"
        )
        interpretation_parts.append("- Consider using non-parametric risk measures")
        interpretation_parts.append(
            "- Portfolio may experience more extreme events than normal models predict"
        )
    else:
        interpretation_parts.append(
            "- Standard risk models (VaR, Sharpe) are appropriate"
        )
        interpretation_parts.append(
...
def _interpret_distribution(
    mean: float, std: float, skew: float, kurt: float, period: str
) -> str:
    """
    ...
    if mean > 0:
        parts.append(
            f"✓ Average {period_name.lower()} return: {mean*100:.2f}% (positive)"
        )
    else:
        parts.append(
            f"⚠ Average {period_name.lower()} return: {mean*100:.2f}% (negative)"
        )
    ...
    if abs(skew) < 0.1:
        ...
        parts.append(
            f"⚠ Left-skewed (skew: {skew:.2f}) - more extreme losses than gains"
        )
    elif skew > 0.5:
        parts.append(
            f"✓ Right-skewed (skew: {skew:.2f}) - more extreme gains than losses"
        )
    else:
    ...
    if kurt < -0.5:
        ...
        parts.append(
            f"⚠ Fat tails (kurtosis: {kurt:.2f}) - more extreme events than normal"
        )
    else:
...
def _interpret_cumulative_returns(
    portfolio_cumulative: pd.Series, benchmark_cumulative: pd.Series = None
) -> str:
    """Interpret cumulative returns chart."""
    parts = []
    parts.append("**Cumulative Returns Analysis:**")

    # Calculate total return
    total_return = (
        portfolio_cumulative.iloc[-1] if not portfolio_cumulative.empty else 0
    )

    # Determine trend
    if total_return > 0.20:
        trend = (
            f"Strong growth: portfolio gained {total_return*100:.1f}% over the period"
        )
    elif total_return > 0.10:
        trend = (
            f"Moderate growth: portfolio gained {total_return*100:.1f}% over the period"
        )
    elif total_return > 0:
        trend = (
            f"Positive return: portfolio gained {total_return*100:.1f}% over the period"
        )
    elif total_return > -0.10:
    ...
    if benchmark_cumulative is not None and not benchmark_cumulative.empty:
        ...
        if abs(outperformance) < 0.01:
            parts.append(
                f"Portfolio performance similar to benchmark ({bench_return*100:.1f}%)"
            )
        elif outperformance > 0.05:
            parts.append(
                f"✓ Outperformed benchmark by {outperformance*100:.1f}% ({bench_return*100:.1f}%)"
            )
        elif outperformance > 0:
            parts.append(
                f"✓ Slightly outperformed benchmark by {outperformance*100:.1f}% ({bench_return*100:.1f}%)"
            )
        elif outperformance > -0.05:
            parts.append(
                f"⚠ Slightly underperformed benchmark by {abs(outperformance)*100:.1f}% ({bench_return*100:.1f}%)"
            )
        else:
            parts.append(
                f"⚠ Underperformed benchmark by {abs(outperformance)*100:.1f}% ({bench_return*100:.1f}%)"
            )
    ...
    if not portfolio_cumulative.empty and len(portfolio_cumulative) > 1:
        ...
        if path_volatility < 0.01:
            parts.append("Smooth growth trajectory with low volatility")
        elif path_volatility < 0.02:
            parts.append("Moderate volatility in growth path")
        else:
            parts.append("Volatile path with significant swings")
...
def _interpret_daily_returns(daily_returns: pd.Series) -> str:
    ...
    parts = []
    parts.append("**Daily Returns Analysis:**")
    ...
    if pos_pct > 60:
        parts.append(
            f"More positive days ({pos_days}, {pos_pct:.0f}%) than negative ({neg_days}, {100-pos_pct:.0f}%)"
        )
    elif pos_pct > 50:
        parts.append(
            f"Slightly more positive days ({pos_days}, {pos_pct:.0f}%) than negative ({neg_days}, {100-pos_pct:.0f}%)"
        )
    elif pos_pct > 40:
        parts.append(
            f"More negative days ({neg_days}, {100-pos_pct:.0f}%) than positive ({pos_days}, {pos_pct:.0f}%)"
        )
    else:
        parts.append(
            f"Significantly more negative days ({neg_days}, {100-pos_pct:.0f}%) than positive ({pos_days}, {pos_pct:.0f}%)"
        )

    # Extreme days
    parts.append(
        f"Largest gain: {max_return*100:.2f}%, Largest loss: {min_return*100:.2f}%"
    )
    ...
    if std_return < 0.01:
        ...
        parts.append(
            f"High volatility ({std_return*100:.2f}% daily) - wide price swings"
        )
...
def _interpret_active_returns(active_returns: pd.Series) -> str:
    ...
    parts = []
    parts.append("**Daily Active Returns Analysis:**")
    ...
    if pos_days_pct > 60:
        parts.append(
            f"Portfolio consistently outperforms benchmark ({pos_days_pct:.0f}% of days positive)"
        )
    elif pos_days_pct > 50:
        parts.append(
            f"Portfolio performance mixed ({pos_days_pct:.0f}% of days positive)"
        )
    else:
        parts.append(
            f"Portfolio consistently underperforms benchmark ({pos_days_pct:.0f}% of days positive)"
        )

    # Volatility
    if std < 0.01:
        parts.append(
            f"Low active return volatility ({std*100:.2f}% daily) - stable alpha"
        )
    elif std < 0.02:
        parts.append(f"Moderate active return volatility ({std*100:.2f}% daily)")
    else:
        parts.append(
            f"High active return volatility ({std*100:.2f}% daily) - volatile alpha"
        )
...
def _interpret_period_returns(periods_df: pd.DataFrame) -> str:
    ...
    parts = []
    parts.append("**Period Returns Analysis:**")
    ...
    best_period = (
        periods_df.loc[best_idx, "Period"]
        if "Period" in periods_df.columns
        else str(best_idx)
    )
    worst_period = (
        periods_df.loc[worst_idx, "Period"]
        if "Period" in periods_df.columns
        else str(worst_idx)
    )
    ...
    if best_return > 0.20:
        parts.append(
            f"Best performing period: {best_period} ({best_return*100:.1f}% - exceptional)"
        )
    elif best_return > 0.10:
        parts.append(
            f"Best performing period: {best_period} ({best_return*100:.1f}% - strong)"
        )
    elif best_return > 0:
        parts.append(
            f"Best performing period: {best_period} ({best_return*100:.1f}% - positive)"
        )
    else:
        parts.append(
            f"Best performing period: {best_period} ({best_return*100:.1f}% - still negative)"
        )

    # Worst period
    if worst_return < -0.20:
        parts.append(
            f"Worst performing period: {worst_period} ({worst_return*100:.1f}% - severe decline)"
        )
    elif worst_return < -0.10:
        parts.append(
            f"Worst performing period: {worst_period} ({worst_return*100:.1f}% - significant decline)"
        )
    elif worst_return < 0:
        parts.append(
            f"Worst performing period: {worst_period} ({worst_return*100:.1f}% - minor decline)"
        )
    else:
        parts.append(
            f"Worst performing period: {worst_period} ({worst_return*100:.1f}% - still positive)"
        )

    # Benchmark comparison if available
    if "Benchmark" in periods_df.columns and "Portfolio" in periods_df.columns:
        aligned = pd.DataFrame(
            {"Portfolio": periods_df["Portfolio"], "Benchmark": periods_df["Benchmark"]}
        ).dropna()
        ...
        if not aligned.empty:
            ...
            outperformed_pct = (
                (outperformed / total_periods * 100) if total_periods > 0 else 0
            )

            if outperformed_pct > 70:
                parts.append(
                    f"Portfolio outperformed benchmark in {outperformed_pct:.0f}% of periods ({outperformed}/{total_periods})"
                )
                ...
                parts.append(
                    f"Portfolio underperformed benchmark in {100-outperformed_pct:.0f}% of periods ({total_periods-outperformed}/{total_periods})"
                )
...
def _interpret_yearly_returns(yearly_df: pd.DataFrame) -> str:
    ...
    parts = []
    parts.append("**Annual Returns Analysis:**")
    ...
    if best_return > 0.30:
        ...
        parts.append(
            f"Best year: {best_year} ({best_return*100:.1f}% - still negative)"
        )

    # Worst year
    if worst_return < -0.30:
        parts.append(
            f"Worst year: {worst_year} ({worst_return*100:.1f}% - severe decline)"
        )
    elif worst_return < -0.20:
        parts.append(
            f"Worst year: {worst_year} ({worst_return*100:.1f}% - significant decline)"
        )
    elif worst_return < 0:
        parts.append(f"Worst year: {worst_year} ({worst_return*100:.1f}% - decline)")
    else:
        parts.append(
            f"Worst year: {worst_year} ({worst_return*100:.1f}% - still positive)"
        )
    ...
    if "Benchmark" in yearly_df.columns:
        aligned = pd.DataFrame(
            {"Portfolio": yearly_df["Portfolio"], "Benchmark": yearly_df["Benchmark"]}
        ).dropna()
        ...
        if not aligned.empty:
            ...
            if outperformed > total_years / 2:
                parts.append(
                    f"Portfolio outperformed benchmark in {outperformed}/{total_years} years"
                )
            elif outperformed > 0:
                parts.append(
                    f"Portfolio outperformed benchmark in {outperformed}/{total_years} years"
                ...
                parts.append(
                    f"Portfolio underperformed benchmark in all {total_years} years"
                )
...
def _interpret_monthly_heatmap(heatmap_df: pd.DataFrame) -> str:
    ...
    best_month_name = (
        month_names[best_month_idx - 1]
        if best_month_idx in range(1, 13)
        else f"Month {best_month_idx}"
    )
    worst_month_name = (
        month_names[worst_month_idx - 1]
        if worst_month_idx in range(1, 13)
        else f"Month {worst_month_idx}"
    )
...
def _interpret_monthly_active_returns(heatmap_df: pd.DataFrame) -> str:
    ...
    parts = []
    parts.append("**Monthly Active Returns Analysis:**")
    ...
    if positive_pct > 60:
        parts.append(
            f"Portfolio consistently outperforms benchmark ({positive_months}/{total_months} positive months)"
        )
    elif positive_pct > 40:
        parts.append(
            f"Portfolio performance mixed ({positive_months}/{total_months} positive months)"
        )
    else:
        parts.append(
            f"Portfolio consistently underperforms benchmark ({positive_months}/{total_months} positive months)"
        )

    # Best/worst months
    parts.append(
        f"Best active month: {best_month_name} ({best_month_avg:.1f}%), Worst: {worst_month_name} ({worst_month_avg:.1f}%)"
    )
...
def _interpret_seasonal_pattern(seasonal_df: pd.DataFrame, pattern_type: str) -> str:
    ...
    if pattern_type == "day_of_week":
        parts.append("**Day of Week Analysis:**")
        pattern_threshold = 0.001  # 0.1% difference
    elif pattern_type == "month":
        parts.append("**Monthly Pattern Analysis:**")
        pattern_threshold = 0.002  # 0.2% difference
    else:  # quarter
        parts.append("**Quarterly Pattern Analysis:**")
        pattern_threshold = 0.003  # 0.3% difference

    # Best
    parts.append(
        f"Best {pattern_type.replace('_', ' ')}: {best_idx} ({best_return*100:.2f}% average)"
    )

    # Worst
    parts.append(
        f"Worst {pattern_type.replace('_', ' ')}: {worst_idx} ({worst_return*100:.2f}% average)"
    )
...
def _interpret_quantiles_box(
    ...
    if benchmark_returns is not None and not benchmark_returns.empty:
        ...
        if q50 > bench_q50 + 0.001:
            parts.append(
                f"Portfolio shows higher returns than benchmark (median: {bench_q50*100:.2f}%)"
            )
        elif q50 < bench_q50 - 0.001:
            parts.append(
                f"Portfolio shows lower returns than benchmark (median: {bench_q50*100:.2f}%)"
            )
        else:
            parts.append(
                f"Portfolio shows similar returns to benchmark (median: {bench_q50*100:.2f}%)"
            )

    # IQR
    parts.append(
        f"Interquartile range: {iqr*100:.2f}% ({q25*100:.2f}% to {q75*100:.2f}%)"
    )

    # Volatility interpretation
    if iqr > 0.05:  # 5% IQR
        parts.append("Wide spread indicates high volatility")
    elif iqr > 0.02:  # 2% IQR
        parts.append("Moderate spread indicates moderate volatility")
    else:
        parts.append("Narrow spread indicates stable returns")
...
def _interpret_win_rate_stats(port_stats: dict, bench_stats: dict = None) -> str:
    ...
    parts = []
    parts.append("**Win Rate Analysis:**")
    ...
    if avg_up_day != 0 and avg_down_day != 0:
        parts.append(
            f"Average win: {avg_up_day:.2f}%, Average loss: {avg_down_day:.2f}%"
        )
        ratio = abs(avg_up_day / avg_down_day) if avg_down_day != 0 else 0
        if ratio > 1.2:
            parts.append(
                f"Positive expectancy (wins are {ratio:.2f}x larger than losses)"
            )
        elif ratio > 0.8:
    ...
    if bench_stats is not None and bench_stats.get("win_rate_daily") is not None:
        ...
        if abs(diff) > 5:
            if diff > 0:
                parts.append(
                    f"Portfolio win rate {diff:.1f}% higher than benchmark ({bench_win_rate:.1f}%)"
                )
            else:
                parts.append(
                    f"Portfolio win rate {abs(diff):.1f}% lower than benchmark ({bench_win_rate:.1f}%)"
                )

    return "\n".join(parts)


def _interpret_rolling_win_rate(
    rolling_win_rate: pd.Series, bench_rolling: pd.Series = None
) -> str:
    """Interpret rolling win rate chart."""
    parts = []
    parts.append("**Rolling Win Rate Analysis:**")
    ...
    if len(rolling_win_rate) > 1:
        recent_avg = (
            rolling_win_rate.iloc[-6:].mean()
            if len(rolling_win_rate) >= 6
            else rolling_win_rate.iloc[-3:].mean()
        )
        early_avg = (
            rolling_win_rate.iloc[:6].mean()
            if len(rolling_win_rate) >= 6
        ...
        if trend > 5:
            ...
            parts.append("Win rate stable over period")
    ...
    if bench_rolling is not None and not bench_rolling.empty:
        ...
        if abs(diff) > 5:
            if diff > 0:
                parts.append(
                    f"Portfolio win rate {diff:.1f}% higher than benchmark ({bench_current:.1f}%)"
                )
            else:
                parts.append(
                    f"Portfolio win rate {abs(diff):.1f}% lower than benchmark ({bench_current:.1f}%)"
                )

    return "\n".join(parts)


def _interpret_smart_ratios(
    ...
    parts = []
    parts.append("**Smart Ratios Analysis:**")
    ...
    # Sharpe adjustment
    parts.append(
        f"Smart Sharpe adjusts for autocorrelation: {sharpe_adjustment:+.2f} difference from observed"
    )
    if abs(sharpe_adjustment) > 0.3:
        parts.append("Autocorrelation significantly impacts Sharpe ratio")
    elif abs(sharpe_adjustment) > 0.1:
        parts.append("Moderate autocorrelation impact on Sharpe ratio")
    else:
        parts.append("Minimal autocorrelation impact on Sharpe ratio")

    # Sortino comparison
    parts.append(
        f"Smart Sortino: {smart_sortino:.2f} vs Observed: {observed_sortino:.2f} (adjustment: {sortino_adjustment:+.2f})"
    )
...
def _interpret_risk_return_scatter(scatter_data: dict) -> str:
    ...
    parts = []
    parts.append("**Risk/Return Analysis:**")
    ...
    # Portfolio position
    parts.append(
        f"Portfolio position: {portfolio_return*100:.1f}% return, {portfolio_risk*100:.1f}% volatility"
    )

    # Benchmark comparison
    if benchmark_return is not None and benchmark_risk is not None:
        parts.append(
            f"Benchmark position: {benchmark_return*100:.1f}% return, {benchmark_risk*100:.1f}% volatility"
        )
        ...
        benchmark_sharpe = (
            (benchmark_return - risk_free_rate) / benchmark_risk
            if benchmark_risk > 0
            else 0
        )
        ...
        if portfolio_sharpe > benchmark_sharpe + 0.1:
            parts.append(
                f"Portfolio offers better risk-adjusted returns (Sharpe: {portfolio_sharpe:.2f} vs {benchmark_sharpe:.2f})"
            )
        elif portfolio_sharpe < benchmark_sharpe - 0.1:
            parts.append(
                f"Portfolio risk-adjusted returns lower than benchmark (Sharpe: {portfolio_sharpe:.2f} vs {benchmark_sharpe:.2f})"
            )
        else:
            parts.append(
                f"Portfolio risk-adjusted returns similar to benchmark (Sharpe: {portfolio_sharpe:.2f} vs {benchmark_sharpe:.2f})"
            )
    ...
    if portfolio_return > 0 and portfolio_risk > 0:
        ...
        if risk_return_ratio > 0.5:
            parts.append("Portfolio appears above the efficient frontier")
        elif risk_return_ratio > 0.3:
            parts.append("Portfolio appears on the efficient frontier")
        else:
            parts.append("Portfolio appears below the efficient frontier")
...
def _interpret_kelly_criterion(kelly_data: dict) -> str:
    ...
    parts = []
    parts.append("**Kelly Criterion Analysis:**")
    ...
    if kelly_full > 0.5:
        parts.append(
            f"Full Kelly ({kelly_full*100:.1f}%) suggests high leverage - very aggressive"
        )
        parts.append(
            f"Half-Kelly ({kelly_half*100:.1f}%) recommended for most investors"
        )
    elif kelly_full > 0.2:
        parts.append(f"Full Kelly ({kelly_full*100:.1f}%) suggests moderate leverage")
        parts.append(
            f"Half-Kelly ({kelly_half*100:.1f}%) provides conservative position sizing"
        )
    elif kelly_full > 0:
        parts.append(f"Full Kelly ({kelly_full*100:.1f}%) suggests low leverage")
        parts.append(
            f"Half-Kelly ({kelly_half*100:.1f}%) provides very conservative position sizing"
        )
    else:
        parts.append("Negative Kelly suggests avoiding this strategy")

    parts.append(
        f"Quarter-Kelly ({kelly_quarter*100:.1f}%) for very risk-averse investors"
    )
...
def _interpret_single_drawdown(dd: dict) -> str:
    ...
    if recovery_days:
        ...
        parts.append("Not yet recovered - ongoing drawdown")

    return "\n".join(parts)


def _interpret_drawdown_chart(
    max_dd: float, current_dd: float, drawdown_series: pd.Series
) -> str:
    """Interpret underwater plot / drawdown chart."""
    parts = []
    parts.append("**Drawdown Analysis:**")

    # Max drawdown
    if max_dd < -0.20:
        parts.append(
            f"⚠ **Severe maximum drawdown:** {max_dd*100:.1f}% - Portfolio lost more than 20% from peak"
        )
    elif max_dd < -0.10:
        parts.append(
            f"⚠ **Significant maximum drawdown:** {max_dd*100:.1f}% - Portfolio lost 10-20% from peak"
        )
    else:
        parts.append(
            f"✓ **Moderate maximum drawdown:** {max_dd*100:.1f}% - Losses stayed below 10%"
        )

    # Current drawdown
    if current_dd < -0.05:
        parts.append(
            f"⚠ **Currently in drawdown:** {current_dd*100:.1f}% - Portfolio is below recent peak"
        )
    elif current_dd < -0.01:
        parts.append(
            f"⚠ **Slight current drawdown:** {current_dd*100:.1f}% - Minor decline from peak"
        )
    else:
        parts.append("✓ **At or near peak:** Portfolio is performing well")
    ...
    if not drawdown_series.empty:
        ...
        if dd_frequency > 20:
            parts.append(
                f"⚠ **Frequent drawdowns:** Portfolio spent {dd_frequency:.1f}% of time in >5% drawdown"
            )
        elif dd_frequency > 10:
            parts.append(
                f"**Moderate drawdown frequency:** Portfolio spent {dd_frequency:.1f}% of time in >5% drawdown"
            )
        else:
            parts.append(
                f"✓ **Low drawdown frequency:** Portfolio spent only {dd_frequency:.1f}% of time in >5% drawdown"
            )

    return "\n".join(parts)


def _interpret_drawdown_periods(
    periods: list, avg_depth: float, avg_duration: float
) -> str:
    ...
    parts.append(
        f"Average depth: {avg_depth*100:.1f}% | Average duration: {avg_duration:.0f} days"
    )
    ...
    if avg_depth < -0.15:
        ...
        parts.append(
            "⚠ Long recovery times - portfolio takes time to recover from losses"
        )
    else:
...
def _interpret_var_distribution(
    var_hist: float, cvar: float, conf_level: float, var_param: float, var_cf: float
) -> str:
    ...
    # Historical VaR
    parts.append(
        f"**Historical VaR:** {var_hist*100:.2f}% - Worst expected loss on {int((1-conf_level)*100)}% of days"
    )

    # CVaR
    parts.append(
        f"**Conditional VaR (Expected Shortfall):** {cvar*100:.2f}% - Average loss on worst days"
    )

    # Method comparison
    if abs(var_hist - var_param) < 0.001:
        parts.append(
            "✓ Historical and Parametric VaR are similar - returns are close to normal"
        )
    elif var_param < var_hist:
        parts.append(
            f"⚠ Parametric VaR ({var_param*100:.2f}%) underestimates risk vs Historical ({var_hist*100:.2f}%)"
        )
    else:
        parts.append(
            f"Parametric VaR ({var_param*100:.2f}%) is more conservative than Historical"
        )

    # Cornish-Fisher
    if abs(var_cf - var_param) > 0.002:
        parts.append(
            f"⚠ Cornish-Fisher VaR ({var_cf*100:.2f}%) differs from Parametric - non-normal distribution detected"
        )
    ...
    if var_hist < -0.05:
        parts.append(
            f"- Portfolio can lose more than 5% on worst {int((1-conf_level)*100)}% of days"
        )
        parts.append(f"- On those worst days, average loss is {cvar*100:.2f}%")
        parts.append("- Consider reducing position sizes or adding hedging")
    else:
        parts.append("- Daily losses typically stay below 5%")
        parts.append("- Risk level is manageable for most investors")

    return "\n".join(parts)


def _interpret_var_benchmark_comparison(
    var_hist: float,
    cvar: float,
    bench_var_hist: float,
    bench_cvar: float,
    ...
    if abs(var_diff) < 0.001:
        parts.append(
            f"Portfolio VaR ({var_hist*100:.2f}%) is similar to benchmark ({bench_var_hist*100:.2f}%) - similar risk levels"
        )
    elif var_diff < 0:
        parts.append(
            f"Portfolio VaR ({var_hist*100:.2f}%) is lower than benchmark ({bench_var_hist*100:.2f}%) by {abs(var_diff_pct):.2f}% - portfolio has lower risk"
        )
    else:
        parts.append(
            f"Portfolio VaR ({var_hist*100:.2f}%) is higher than benchmark ({bench_var_hist*100:.2f}%) by {var_diff_pct:.2f}% - portfolio has higher risk"
        )
    ...
    if abs(cvar_diff) < 0.001:
        parts.append(
            f"Portfolio CVaR ({cvar*100:.2f}%) is similar to benchmark ({bench_cvar*100:.2f}%) - similar tail risk"
        )
    elif cvar_diff < 0:
        parts.append(
            f"Portfolio CVaR ({cvar*100:.2f}%) is lower than benchmark ({bench_cvar*100:.2f}%) by {abs(cvar_diff_pct):.2f}% - portfolio has lower tail risk"
        )
    else:
        parts.append(
            f"Portfolio CVaR ({cvar*100:.2f}%) is higher than benchmark ({bench_cvar*100:.2f}%) by {cvar_diff_pct:.2f}% - portfolio has higher tail risk"
        )

    return "\n".join(parts)


def _interpret_rolling_metric(
    ...
    parts.append(
        f"Current: {current:.3f} | Average: {mean_val:.3f} | Range: {min_val:.3f} to {max_val:.3f}"
    )
    ...
    if len(metric_series) > 10:
        ...
        trend = (
            "improving"
            if (direction == "higher" and recent_avg > earlier_avg)
            or (direction == "lower" and recent_avg < earlier_avg)
            else "declining"
        )
        parts.append(
            f"Trend: {trend} (recent average: {recent_avg:.3f} vs earlier: {earlier_avg:.3f})"
        )

    # Threshold comparison
    if threshold is not None:
        above_threshold = (
            (metric_series > threshold).sum() / len(metric_series) * 100
            if direction == "higher"
            else (metric_series < threshold).sum() / len(metric_series) * 100
        )
        if direction == "higher":
...
def _interpret_rolling_beta(beta_series: pd.Series, window: int) -> str:
    ...
    if abs(current - 1.0) < 0.1:
        parts.append(
            f"Current beta ({current:.3f}) is close to 1.0 - portfolio moves similarly to benchmark"
        )
    elif current > 1.0:
        parts.append(
            f"Current beta ({current:.3f}) > 1.0 - portfolio is more volatile than benchmark"
        )
    else:
        parts.append(
            f"Current beta ({current:.3f}) < 1.0 - portfolio is less volatile than benchmark"
        )
    ...
    if len(beta_series) > 10:
        ...
        if abs(recent_avg - earlier_avg) < 0.05:
            parts.append(
                f"Beta is relatively stable (recent: {recent_avg:.3f} vs earlier: {earlier_avg:.3f})"
            )
        elif recent_avg > earlier_avg:
            parts.append(
                f"Beta is increasing (recent: {recent_avg:.3f} vs earlier: {earlier_avg:.3f}) - portfolio becoming more sensitive to market"
            )
        else:
            parts.append(
                f"Beta is decreasing (recent: {recent_avg:.3f} vs earlier: {earlier_avg:.3f}) - portfolio becoming less sensitive to market"
            )
...
def _interpret_bull_bear_analysis(bull: dict, bear: dict) -> str:
    ...
    if abs(return_diff) < 0.01:
        parts.append(
            f"Portfolio performs similarly in bull ({bull_port_return:.2f}%) and bear ({bear_port_return:.2f}%) markets"
        )
    elif return_diff > 0:
        parts.append(
            f"Portfolio performs better in bull markets ({bull_port_return:.2f}%) than in bear markets ({bear_port_return:.2f}%) - difference: {return_diff:.2f}%"
        )
    else:
        parts.append(
            f"Portfolio performs better in bear markets ({bear_port_return:.2f}%) than in bull markets ({bull_port_return:.2f}%) - difference: {abs(return_diff):.2f}%"
        )
    ...
    if abs(beta_diff) < 0.1:
        parts.append(
            f"Beta is similar in bull ({bull_beta:.2f}) and bear ({bear_beta:.2f}) markets - consistent market sensitivity"
        )
    elif beta_diff > 0:
        parts.append(
            f"Beta is higher in bull markets ({bull_beta:.2f}) than in bear markets ({bear_beta:.2f}) - more sensitive during uptrends"
        )
    else:
        parts.append(
            f"Beta is higher in bear markets ({bear_beta:.2f}) than in bull markets ({bull_beta:.2f}) - more sensitive during downtrends"
        )
    ...
    if bull_outperf > 0 and bear_outperf > 0:
        parts.append(
            f"Portfolio outperforms benchmark in both bull ({bull_outperf:.2f}%) and bear ({bear_outperf:.2f}%) markets"
        )
    elif bull_outperf > 0:
        parts.append(
            f"Portfolio outperforms in bull markets ({bull_outperf:.2f}%) but underperforms in bear markets ({bear_outperf:.2f}%)"
        )
    elif bear_outperf > 0:
        parts.append(
            f"Portfolio underperforms in bull markets ({bull_outperf:.2f}%) but outperforms in bear markets ({bear_outperf:.2f}%)"
        )
    else:
        parts.append(
            f"Portfolio underperforms benchmark in both bull ({bull_outperf:.2f}%) and bear ({bear_outperf:.2f}%) markets"
        )
...
def _interpret_impact_on_return(impact_data: dict) -> str:
    ...
    parts.append("**Impact on Total Return Analysis:**")
    parts.append(
        f"Top contributor: {top_ticker} ({top_contrib:.2f}% of portfolio return)"
    )
    ...
    if top_contrib_pct > 50:
        parts.append(
            f"⚠ High concentration: {top_ticker} accounts for {top_contrib_pct:.1f}% of total contribution"
        )
        parts.append("Portfolio return is heavily dependent on single asset")
    elif top_contrib_pct > 30:
        parts.append(
            f"Moderate concentration: {top_ticker} accounts for {top_contrib_pct:.1f}% of total contribution"
        )
    else:
        parts.append(
            f"✓ Well-distributed: top contributor accounts for {top_contrib_pct:.1f}% of total contribution"
        )
    ...
    if len(tickers) >= 3:
        ...
        parts.append(
            f"Top 3 assets account for {top3_pct:.1f}% of total return contribution"
        )
...
def _interpret_impact_on_risk(impact_data: dict) -> str:
    ...
    parts.append("**Impact on Portfolio Risk Analysis:**")
    parts.append(
        f"Biggest risk contributor: {top_ticker} ({top_risk:.1f}% of portfolio risk)"
    )
    ...
    if top_risk_pct > 50:
        parts.append(
            f"⚠ High risk concentration: {top_ticker} accounts for {top_risk_pct:.1f}% of total risk"
        )
        parts.append(
            "Portfolio risk is heavily dependent on single asset - consider diversification"
        )
    elif top_risk_pct > 30:
        parts.append(
            f"Moderate risk concentration: {top_ticker} accounts for {top_risk_pct:.1f}% of total risk"
        )
    else:
        parts.append(
            f"✓ Well-distributed risk: top contributor accounts for {top_risk_pct:.1f}% of total risk"
        )
    ...
    if len(tickers) >= 3:
        ...
        parts.append(
            f"Top 3 assets account for {top3_pct:.1f}% of total risk contribution"
        )
...
def _interpret_risk_vs_weight_comparison(comparison_data: dict) -> str:
    ...
    parts.append("**Risk vs Weight Comparison Analysis:**")
    ...
    if imbalances:
        ...
        if high_risk:
            ...
            parts.append(
                f"⚠ {top_imbalance[0]}: Risk impact ({top_imbalance[3]:.1f}%) >> Weight ({top_imbalance[4]:.1f}%) - {top_imbalance[2]:.1f}x higher"
            )
            parts.append(
                "This asset contributes more risk than its portfolio weight suggests"
            )

    if return_imbalances:
        top_return = return_imbalances[0]
        parts.append(
            f"✓ {top_return[0]}: Return impact ({top_return[2]:.2f}%) >> Weight ({top_return[3]:.1f}%) - {top_return[1]:.1f}x higher"
        )
        parts.append(
            "This asset contributes more return than its portfolio weight suggests"
        )

    if not imbalances and not return_imbalances:
        parts.append(
            "✓ Portfolio shows balanced risk/return distribution relative to weights"
        )
        parts.append("Assets contribute proportionally to their weights")
...
def _interpret_correlation_matrix(corr_matrix_data: dict) -> str:
    ...
    parts.append("**Correlation Matrix Analysis:**")

    # Overall assessment
    if avg_corr < 0.3:
        parts.append(
            f"✓ Low average correlation ({avg_corr:.2f}) - Excellent diversification potential"
        )
    elif avg_corr < 0.5:
        parts.append(
            f"Moderate average correlation ({avg_corr:.2f}) - Good diversification potential"
        )
    else:
        parts.append(
    ...
    if high_corr > 0:
        parts.append(
            f"⚠ Found {high_corr} pair(s) with correlation > 0.8 - Risk of concentration"
        )
    else:
        parts.append("✓ No highly correlated pairs (>0.8) found")

    # Low correlation pairs
    if low_corr > 0:
        parts.append(
            f"✓ Found {low_corr} pair(s) with correlation < 0.2 - Good diversification opportunities"
        )
    else:
        parts.append(
            "⚠ No low correlation pairs (<0.2) found - Consider adding assets with lower correlation"
        )
...
def _interpret_correlation_with_benchmark(corr_bench_data: dict) -> str:
    ...
    parts.append("**Correlation with Benchmark Analysis:**")
    parts.append(f"Average correlation: {avg_corr:.2f}")

    # Highest correlation
    parts.append(
        f"Highest correlation: {tickers[max_idx]} ({correlations[max_idx]:.2f})"
    )
    if betas and max_idx < len(betas):
        parts.append(
            f"Beta: {betas[max_idx]:.2f} - {'More volatile' if betas[max_idx] > 1.0 else 'Less volatile'} than benchmark"
        )

    # Lowest correlation
    if min_idx != max_idx:
        parts.append(
            f"Lowest correlation: {tickers[min_idx]} ({correlations[min_idx]:.2f})"
        )
        if betas and min_idx < len(betas):
            parts.append(
                f"Beta: {betas[min_idx]:.2f} - {'More volatile' if betas[min_idx] > 1.0 else 'Less volatile'} than benchmark"
            )

    # Overall assessment
    if avg_corr > 0.8:
        parts.append("⚠ Portfolio is highly sensitive to benchmark movements")
    elif avg_corr > 0.5:
        parts.append("Portfolio shows moderate sensitivity to benchmark")
    else:
        parts.append(
            "✓ Portfolio shows low sensitivity to benchmark - good diversification"
        )
...
def _interpret_average_correlation_to_portfolio(avg_corr_data: dict) -> str:
    ...
    if (
        not tickers
        or not avg_correlations
        or len(tickers) == 0
        or len(avg_correlations) == 0
    ):
        return ""
    ...
    parts.append("**Average Correlation to Portfolio Analysis:**")

    # Highest correlation (worst for diversification)
    parts.append(
        f"⚠ Highest average correlation: {tickers[max_idx]} ({avg_correlations[max_idx]:.3f})"
    )
    if diversification_scores and max_idx < len(diversification_scores):
        parts.append(
            f"Diversification score: {diversification_scores[max_idx]:.3f} - Consider reducing weight"
        )

    # Lowest correlation (best for diversification)
    if min_idx != max_idx:
        parts.append(
            f"✓ Lowest average correlation: {tickers[min_idx]} ({avg_correlations[min_idx]:.3f})"
        )
        if diversification_scores and min_idx < len(diversification_scores):
            parts.append(
                f"Diversification score: {diversification_scores[min_idx]:.3f} - Good for diversification"
            )
    ...
    if good_div > 0:
        parts.append(
            f"✓ {good_div} asset(s) with good diversification (correlation < 0.3)"
        )
    if bad_div > 0:
        parts.append(
            f"⚠ {bad_div} asset(s) with poor diversification (correlation > 0.7) - Consider reducing exposure"
        )
...
def _interpret_rolling_correlations(rolling_corr_data: dict, window: int) -> str:
    ...
    if not rolling_correlations or (
        isinstance(rolling_correlations, dict) and len(rolling_correlations) == 0
    ):
        return ""
    ...
    parts.append(
        f"Highest average correlation: {top_pair['pair']} ({top_pair['mean']:.2f})"
    )
    ...
    if top_pair["stable"]:
        ...
        parts.append(
            f"⚠ Correlation is volatile (std: {top_pair['std']:.2f}) - Range: {top_pair['min']:.2f} to {top_pair['max']:.2f}"
        )
    ...
    if high_spikes:
        parts.append(
            f"⚠ {len(high_spikes)} pair(s) show correlation spikes during market stress"
        )
    ...
    if len(stability_analysis) > 0:
        ...
        if avg_mean < 0.3:
            parts.append(
                "✓ Overall low correlations - Good diversification maintained over time"
            )
        elif avg_mean > 0.7:
            parts.append(
                "⚠ Overall high correlations - Limited diversification over time"
            )
...
def _interpret_asset_price_dynamics(price_dynamics_data: dict) -> str:
    ...
    parts.append("**Asset Price Dynamics Analysis:**")
    ...
    if spread > 50:
        parts.append(
            f"⚠ Large performance spread ({spread:.2f}%) - High dispersion in asset returns"
        )
    elif spread > 20:
        parts.append(f"Moderate performance spread ({spread:.2f}%)")
    else:
        parts.append(
            f"✓ Relatively tight performance spread ({spread:.2f}%) - Assets moving similarly"
        )
    ...
    if positive_count == len(final_returns):
        parts.append("✓ All assets show positive returns")
    elif negative_count == len(final_returns):
        parts.append("⚠ All assets show negative returns")
    else:
        parts.append(
            f"Mixed performance: {positive_count} positive, {negative_count} negative"
        )

    return "\n".join(parts)


def _interpret_rolling_correlation_with_benchmark(
    rolling_corr_data: dict, window: int
) -> str:
    ...
    parts.append(
        f"**Rolling Correlation with Benchmark Analysis ({window}-day window):**"
    )
    ...
    parts.append(
        f"Highest average correlation: {top_asset['ticker']} ({top_asset['mean']:.2f})"
    )
    ...
    if top_asset["stable"]:
        ...
        parts.append(
            f"⚠ Correlation is volatile (std: {top_asset['std']:.2f}) - Range: {top_asset['min']:.2f} to {top_asset['max']:.2f}"
        )
    ...
    if avg_mean > 0.8:
        parts.append(
            "⚠ Portfolio assets are highly correlated with benchmark - Limited diversification"
        )
    elif avg_mean > 0.5:
        parts.append("Moderate correlation with benchmark")
    else:
        parts.append("✓ Low correlation with benchmark - Good diversification")
...
def _interpret_price_volume_chart(detailed_data: dict, ticker: str) -> str:
    ...
    if price_start and price_end and price_start > 0:
        ...
        if price_change > 20:
            ...
            parts.append("Moderate upward trend")
        elif price_change < -20:
            parts.append("⚠ Strong downward trend")
        elif price_change < -5:
            parts.append("Moderate downward trend")
        else:
            parts.append("Relatively flat price movement")
        ...
        if (
            ...
            if current_ma50 and current_ma200:
                if current_price > current_ma50 > current_ma200:
                    parts.append("✓ Price above both MA50 and MA200 - Bullish signal")
                elif current_price < current_ma50 < current_ma200:
                    parts.append("⚠ Price below both MA50 and MA200 - Bearish signal")
                elif current_price > current_ma50:
                    parts.append("Price above MA50 but below MA200 - Mixed signal")
                else:
                    parts.append("Price below MA50 - Potential weakness")
...
def _interpret_comparison_of_return(detailed_data: dict, ticker: str) -> str:
    ...
    if portfolio_cum is not None and not portfolio_cum.empty:
        ...
        if abs(diff_portfolio) < 1:
            parts.append(
                f"Asset performance similar to portfolio ({diff_portfolio:+.2f}% difference)"
            )
        elif diff_portfolio > 0:
            parts.append(f"✓ Asset outperforms portfolio by {diff_portfolio:.2f}%")
        else:
            parts.append(
                f"⚠ Asset underperforms portfolio by {abs(diff_portfolio):.2f}%"
            )
    ...
    if benchmark_cum is not None and not benchmark_cum.empty:
        ...
        if abs(diff_benchmark) < 1:
            parts.append(
                f"Asset performance similar to benchmark ({diff_benchmark:+.2f}% difference)"
            )
        elif diff_benchmark > 0:
            parts.append(f"✓ Asset outperforms benchmark by {diff_benchmark:.2f}%")
        else:
            parts.append(
                f"⚠ Asset underperforms benchmark by {abs(diff_benchmark):.2f}%"
            )
...
def _interpret_asset_correlations(other_corrs: dict, ticker: str) -> str:
    ...
    if highest_corr > 0.8:
        parts.append(
            "⚠ Very high correlation - Assets move together, limited diversification benefit"
        )
    elif highest_corr > 0.5:
        parts.append("Moderate correlation - Some diversification benefit")
    else:
        parts.append("✓ Low correlation - Good diversification potential")
    ...
    if len(sorted_corrs) > 1:
        ...
        if lowest_corr < 0:
            parts.append("✓ Negative correlation - Natural hedging opportunity")
        elif lowest_corr < 0.2:
            parts.append("✓ Very low correlation - Excellent diversification")
    ...
    if high_corr_count > 0:
        parts.append(
            f"⚠ {high_corr_count} asset(s) with high correlation (>0.7) - Consider reducing exposure to similar assets"
        )
    if low_corr_count > 0:
        parts.append(
            f"✓ {low_corr_count} asset(s) with low correlation (<0.3) - Good diversification opportunities"
        )
...
def show():
    ...
    with col2:
        ...
        if cmp_type == "Index ETF":
            ...
            comparison_value = st.selectbox(
                "Index ETF", options=presets, key="cmp_etf_select"
            )
            comparison_type = "ticker"
        elif cmp_type == "Portfolio":
            other_names = [
                name for name in portfolio_names.keys() if name != selected_name
            ]
            if other_names:
                sel_other = st.selectbox(
                    "Portfolio", options=other_names, key="cmp_portfolio_select"
                )
    ...
    benchmark_returns = (
        ...
        else analytics.get("benchmark_returns")
    )
    ...
    tab1, tab2, tab3, tab4, tab5 = st.tabs(
        ["Overview", "Performance", "Risk", "Assets & Correlations", "Export & Reports"]
    )
...
def _render_overview_tab(
    ...
    if benchmark_returns is not None and not benchmark_returns.empty:
        ...
        bm_for_cards = {
            ...
            "annualized_return": float(
                calculate_annualized_return(aligned_bench_cards)
            ),
            "volatility": float(
                calculate_volatility(aligned_bench_cards).get("annual", 0.0)
            ),
            "max_drawdown": float(
                max_dd_cards[0] if isinstance(max_dd_cards, tuple) else max_dd_cards
            ),
            "sharpe_ratio": float(
                calculate_sharpe_ratio(
                    aligned_bench_cards, risk_free_rate=risk_free_rate
                )
                or 0
            ),
            "sortino_ratio": float(
    ...
    portfolio_metrics_flat = {
        ...
        "volatility": (
            (risk.get("volatility", {}) or {}).get("annual", risk.get("volatility", 0))
            if isinstance(risk.get("volatility", 0), dict)
            else risk.get("volatility", 0)
        ),
    ...
    try:
        if portfolio_returns is not None and not portfolio_returns.empty:
            ...
            if abs(portfolio_metrics_flat.get("total_return", 0)) < 1e-8:
                portfolio_metrics_flat["total_return"] = float(
            ...
            portfolio_metrics_flat["cagr"] = portfolio_metrics_flat.get(
                "annualized_return", portfolio_metrics_flat.get("cagr", 0)
            )
            ...
            try:
                ...
                if isinstance(vol, dict):
                    ...
                    vol_val = float(vol)
                if vol_val == 0 or portfolio_metrics_flat.get("volatility") in (
                    0,
                    None,
                ):
    ...
    if portfolio_returns is not None and not portfolio_returns.empty:
        ...
        if cum_data:
            ...
            st.plotly_chart(
                fig, use_container_width=True, key="overview_cumulative_returns"
            )
            ...
            if portfolio_cum is not None and not portfolio_cum.empty:
                interpretation = _interpret_cumulative_returns(
        ...
        if portfolio_values is not None and not portfolio_values.empty:
            ...
            if underwater_data:
                ...
                st.plotly_chart(
                    fig, use_container_width=True, key="overview_underwater"
                )
                ...
                if not portfolio_dd.empty:
                    ...
                    current_dd = (
                        portfolio_dd.iloc[-1] / 100 if len(portfolio_dd) > 0 else 0
                    )
                    interpretation = _interpret_drawdown_chart(
                        max_dd, current_dd, portfolio_dd
                    )
                    st.info(interpretation)
    ...
    with col1:
        ...
        if positions:
            ...
            if total_weight > 0:
                ...
                st.plotly_chart(
                    fig, use_container_width=True, key="overview_asset_allocation"
                )
                ...
                if alloc_data:
                    ...
                    st.caption(
                        f"**Top asset:** {top_ticker} - {top_weight:.1f}% of portfolio"
                    )
        else:
    ...
    with col2:
        ...
        if sector_to_weight:
            ...
            st.plotly_chart(
                fig, use_container_width=True, key="overview_sector_allocation"
            )
            ...
            st.caption(
                f"**Top sector:** {top_sector} - {top_sector_weight:.1f}% of portfolio"
            )
        else:
        ...
        try:
            ...
            if not aligned_bench.empty:
                ...
                benchmark_metrics_flat = {
                    ...
                    "volatility": float(
                        calculate_volatility(aligned_bench).get("annual", 0.0)
                    ),
...
def _render_performance_tab(
    ...
    sub_tab1, sub_tab2, sub_tab3 = st.tabs(
        ["Returns Analysis", "Periodic Analysis", "Distribution"]
    )
...
def _render_returns_analysis(
    perf, portfolio_returns, benchmark_returns, portfolio_values, benchmark_values
):
    ...
    if benchmark_returns is not None and not benchmark_returns.empty:
        ...
        st.plotly_chart(
            fig, use_container_width=True, key="returns_active_returns_area"
        )
        ...
        with col3:
            st.metric(
                "Negative Days",
                f"{total_days - pos_days} ({(total_days-pos_days)/total_days*100:.1f}%)",
            )
        with col4:
    ...
    if benchmark_returns is not None and not benchmark_returns.empty:
        cpp_data = calculate_common_performance_periods(
        ...
        if cpp_data:
            ...
            correlation_level = (
                "highly"
                if cpp_index > 0.7
                else "moderately" if cpp_index > 0.4 else "lowly"
            )
            st.info(
                f"Portfolio is {correlation_level} correlated with market direction."
            )
    ...
    if rolling_periods.get("best") is not None and not rolling_periods["best"].empty:
        ...
        best_df["Start"] = best_df["Start"].apply(
            lambda x: x.strftime("%Y-%m-%d") if hasattr(x, "strftime") else str(x)
        )
        best_df["End"] = best_df["End"].apply(
            lambda x: x.strftime("%Y-%m-%d") if hasattr(x, "strftime") else str(x)
        )
        display_best = best_df[
            ["#", "Start", "End", "Portfolio", "Benchmark", "Difference"]
        ]
    ...
    if rolling_periods.get("worst") is not None and not rolling_periods["worst"].empty:
        ...
        worst_df["Start"] = worst_df["Start"].apply(
            lambda x: x.strftime("%Y-%m-%d") if hasattr(x, "strftime") else str(x)
        )
        worst_df["End"] = worst_df["End"].apply(
            lambda x: x.strftime("%Y-%m-%d") if hasattr(x, "strftime") else str(x)
        )
        display_worst = worst_df[
            ["#", "Start", "End", "Portfolio", "Benchmark", "Difference"]
        ]
...
def _render_periodic_analysis(portfolio_returns, benchmark_returns):
    ...
    if benchmark_returns is not None and not benchmark_returns.empty:
        ...
        active_heatmap_data = get_monthly_active_returns_data(
            portfolio_returns, benchmark_returns
        )
        if (
            active_heatmap_data.get("heatmap") is not None
            and not active_heatmap_data["heatmap"].empty
        ):
            fig = plot_monthly_heatmap({"heatmap": active_heatmap_data["heatmap"]})
            st.plotly_chart(
                fig, use_container_width=True, key="periodic_monthly_active"
            )

            # Automatic interpretation
            interpretation = _interpret_monthly_active_returns(
                active_heatmap_data["heatmap"]
            )
            st.info(interpretation)
    ...
    if (
        seasonal_data.get("day_of_week") is not None
        and not seasonal_data["day_of_week"].empty
    ):
        col1, col2, col3 = st.columns(3)

        with col1:
            fig = plot_seasonal_bar(
                seasonal_data["day_of_week"], "Avg Return by Day of Week (%)"
            )
            st.plotly_chart(fig, use_container_width=True, key="seasonal_day")

            # Automatic interpretation
            interpretation = _interpret_seasonal_pattern(
                seasonal_data["day_of_week"], "day_of_week"
            )
            st.info(interpretation)
        ...
        with col3:
            fig = plot_seasonal_bar(
                seasonal_data["quarter"], "Avg Return by Quarter (%)"
            )
            st.plotly_chart(fig, use_container_width=True, key="seasonal_quarter")

            # Automatic interpretation
            interpretation = _interpret_seasonal_pattern(
                seasonal_data["quarter"], "quarter"
            )
            st.info(interpretation)
...
def _render_distribution_analysis(portfolio_returns, benchmark_returns):
    ...
    with col2:
        ...
        if not monthly_returns.empty:
            ...
            if dist_data_monthly:
                ...
                st.plotly_chart(
                    fig, use_container_width=True, key="distribution_monthly"
                )
                ...
                if dist_data_monthly.get("mean") is not None:
                    ...
                    interpretation = _interpret_distribution(
                        mean, std, skew, kurt, "monthly"
                    )
                    st.info(interpretation)
    ...
    if qq_data:
        ...
        if len(theoretical) > 0 and len(sample) > 0:
            ...
            # Interpretation
            interpretation = _interpret_qq_plot(
                deviations, lower_tail, upper_tail, mean_dev, std_dev
            )
            st.info(interpretation)
    ...
    if win_rate_data.get("stats"):
        ...
        bench_values = [
            (
                f"{bench_stats.get('win_rate_daily', 0)*100:.1f}%"
                if bench_stats.get("win_rate_daily") is not None
                else "N/A"
            ),
            (
                f"{bench_stats.get('win_rate_weekly', 0)*100:.1f}%"
                if bench_stats.get("win_rate_weekly") is not None
                else "N/A"
            ),
            (
                f"{bench_stats.get('win_rate_monthly', 0)*100:.1f}%"
                if bench_stats.get("win_rate_monthly") is not None
                else "N/A"
            ),
            (
                f"{bench_stats.get('win_rate_quarterly', 0)*100:.1f}%"
                if bench_stats.get("win_rate_quarterly") is not None
                else "N/A"
            ),
            (
                f"{bench_stats.get('win_rate_yearly', 0)*100:.1f}%"
                if bench_stats.get("win_rate_yearly") is not None
                else "N/A"
            ),
            (
                f"{bench_stats.get('avg_win_daily', 0)*100:.2f}%"
                if bench_stats.get("avg_win_daily") is not None
                else "N/A"
            ),
            (
                f"{bench_stats.get('avg_loss_daily', 0)*100:.2f}%"
                if bench_stats.get("avg_loss_daily") is not None
                else "N/A"
            ),
            (
                f"{bench_stats.get('avg_win_monthly', 0)*100:.2f}%"
                if bench_stats.get("avg_win_monthly") is not None
                else "N/A"
            ),
            (
                f"{bench_stats.get('avg_loss_monthly', 0)*100:.2f}%"
                if bench_stats.get("avg_loss_monthly") is not None
                else "N/A"
            ),
            (
                f"{bench_stats.get('best_daily', 0)*100:.2f}%"
                if bench_stats.get("best_daily") is not None
                else "N/A"
            ),
            (
                f"{bench_stats.get('worst_daily', 0)*100:.2f}%"
                if bench_stats.get("worst_daily") is not None
                else "N/A"
            ),
            (
                f"{bench_stats.get('best_monthly', 0)*100:.2f}%"
                if bench_stats.get("best_monthly") is not None
                else "N/A"
            ),
            (
                f"{bench_stats.get('worst_monthly', 0)*100:.2f}%"
                if bench_stats.get("worst_monthly") is not None
                else "N/A"
            ),
        ]
        ...
        bench_vals = [
            (
                bench_stats.get("win_rate_daily", 0) * 100
                if bench_stats.get("win_rate_daily") is not None
                else None
            ),
            (
                bench_stats.get("win_rate_weekly", 0) * 100
                if bench_stats.get("win_rate_weekly") is not None
                else None
            ),
            (
                bench_stats.get("win_rate_monthly", 0) * 100
                if bench_stats.get("win_rate_monthly") is not None
                else None
            ),
            (
                bench_stats.get("win_rate_quarterly", 0) * 100
                if bench_stats.get("win_rate_quarterly") is not None
                else None
            ),
            (
                bench_stats.get("win_rate_yearly", 0) * 100
                if bench_stats.get("win_rate_yearly") is not None
                else None
            ),
            (
                bench_stats.get("avg_win_daily", 0) * 100
                if bench_stats.get("avg_win_daily") is not None
                else None
            ),
            (
                bench_stats.get("avg_loss_daily", 0) * 100
                if bench_stats.get("avg_loss_daily") is not None
                else None
            ),
            (
                bench_stats.get("avg_win_monthly", 0) * 100
                if bench_stats.get("avg_win_monthly") is not None
                else None
            ),
            (
                bench_stats.get("avg_loss_monthly", 0) * 100
                if bench_stats.get("avg_loss_monthly") is not None
                else None
            ),
            (
                bench_stats.get("best_daily", 0) * 100
                if bench_stats.get("best_daily") is not None
                else None
            ),
            (
                bench_stats.get("worst_daily", 0) * 100
                if bench_stats.get("worst_daily") is not None
                else None
            ),
            (
                bench_stats.get("best_monthly", 0) * 100
                if bench_stats.get("best_monthly") is not None
                else None
            ),
            (
                bench_stats.get("worst_monthly", 0) * 100
                if bench_stats.get("worst_monthly") is not None
                else None
            ),
        ]
        ...
        st.dataframe(
            win_rate_df, use_container_width=True, hide_index=True, height=total_height
        )
        ...
        if not rolling_win_rate.empty:
            ...
            st.plotly_chart(
                fig, use_container_width=True, key="distribution_rolling_win_rate"
            )

            # Automatic interpretation
            interpretation = _interpret_rolling_win_rate(
                rolling_win_rate, bench_rolling
            )
            st.info(interpretation)
    ...
    if outlier_data.get("stats"):
        ...
        st.plotly_chart(
            fig, use_container_width=True, key="distribution_outlier_scatter"
        )
    ...
    if stats_tests:
        ...
        if sample_size > 1000:
            st.caption(
                ...
                + (
                    f" (Shapiro-Wilk used random sample of {shapiro_sample:,})"
                    if shapiro_sample < sample_size
                    else ""
                )
            )
        ...
        skew_interpretation = (
            ...
            if skewness < -0.1
            else "Slight positive skew" if skewness > 0.1 else "Symmetric"
        )
        kurtosis_interpretation = (
            "Leptokurtic (fat tails)"
            if kurtosis > 0.5
            else "Platykurtic (thin tails)" if kurtosis < -0.5 else "Normal tails"
        )
...
def _render_risk_tab(
    ...
    sub_tab1, sub_tab2, sub_tab3, sub_tab4 = st.tabs(
        ["Key Metrics", "Drawdown Analysis", "VaR & CVaR", "Rolling Risk Metrics"]
    )
    ...
    with sub_tab1:
        _render_risk_key_metrics(
            risk, ratios, market, benchmark_returns, portfolio_returns, risk_free_rate
        )

    with sub_tab2:
        _render_drawdown_analysis(
            risk,
            portfolio_values,
            benchmark_values,
            portfolio_returns,
...
def _render_risk_key_metrics(
    ...
    portfolio_max_dd = (
        portfolio_max_dd_tuple[0]
        if isinstance(portfolio_max_dd_tuple, tuple)
        else portfolio_max_dd_tuple
    )
    ...
    if benchmark_returns is not None and not benchmark_returns.empty:
        try:
            ...
            bench_vol_annual = (
                bench_vol.get("annual", 0.0)
                if isinstance(bench_vol, dict)
                else bench_vol
            )
            bench_max_dd = calculate_max_drawdown(benchmark_returns)
            bench_max_dd_val = (
                bench_max_dd[0] if isinstance(bench_max_dd, tuple) else bench_max_dd
            )
            ...
            benchmark_risk_metrics = {
                ...
                "sortino_ratio": float(
                    calculate_sortino_ratio(benchmark_returns, risk_free_rate) or 0
                ),
                "calmar_ratio": float(calculate_calmar_ratio(benchmark_returns) or 0),
                "var_95": float(
                    calculate_var(
                        benchmark_returns, confidence_level=0.95, method="historical"
                    )
                    or 0
                ),
                "cvar_95": float(
                    calculate_cvar(benchmark_returns, confidence_level=0.95) or 0
                ),
                "up_capture": 1.0,
                "down_capture": 1.0,
            }
    ...
    vol_annual = (
        portfolio_vol.get("annual", 0.0)
        if isinstance(portfolio_vol, dict)
        else portfolio_vol
    )
    ...
    psr_95 = calculate_probabilistic_sharpe_ratio(
        portfolio_returns, risk_free_rate, benchmark_sharpe=1.0
    )
    psr_99 = calculate_probabilistic_sharpe_ratio(
        portfolio_returns, risk_free_rate, benchmark_sharpe=0.0
    )
    ...
    if smart_sharpe is not None and smart_sortino is not None:
        ...
        st.dataframe(
            smart_ratios_df, use_container_width=True, hide_index=True, height=230
        )
        ...
        if observed_sharpe is not None and smart_sharpe is not None:
            interpretation = _interpret_smart_ratios(
                observed_sharpe, smart_sharpe, observed_sortino, smart_sortino
            )
            st.info(interpretation)
    else:
    ...
    if benchmark_returns is not None and not benchmark_returns.empty:
        ...
        try:
            ...
            tracking_error = (
                calculate_tracking_error(portfolio_returns, benchmark_returns) or 0
            )
            info_ratio = (
                calculate_information_ratio(portfolio_returns, benchmark_returns) or 0
            )
            ...
            active_return_pct = (
                abs(active_return) / abs(port_return) * 100 if port_return != 0 else 0
            )
            if info_ratio > 1.0:
                st.success(
    ...
    if kelly_data:
        ...
        with col2:
            if risk_of_ruin:
                ...
                st.dataframe(
                    ruin_df, use_container_width=True, hide_index=True, height=210
                )
    ...
    port_dd_val = (
        port_dd_tuple[0] if isinstance(port_dd_tuple, tuple) else port_dd_tuple
    )
    port_dd_date = (
        port_dd_tuple[1]
        if isinstance(port_dd_tuple, tuple) and len(port_dd_tuple) > 1
        else None
    )
    port_dd_trough = (
        port_dd_tuple[2]
        if isinstance(port_dd_tuple, tuple) and len(port_dd_tuple) > 2
        else None
    )
    port_dd_duration = (
        port_dd_tuple[3]
        if isinstance(port_dd_tuple, tuple) and len(port_dd_tuple) > 3
        else None
    )
    ...
    port_var_95_cf = (
        calculate_var(portfolio_returns, 0.95, method="cornish_fisher") or 0
    )
    ...
    if benchmark_returns is not None and not benchmark_returns.empty:
        try:
            ...
            bench_dd_val = (
                bench_dd_tuple[0]
                if isinstance(bench_dd_tuple, tuple)
                else bench_dd_tuple
            )
            bench_dd_date = (
                bench_dd_tuple[1]
                if isinstance(bench_dd_tuple, tuple) and len(bench_dd_tuple) > 1
                else None
            )
            bench_dd_trough = (
                bench_dd_tuple[2]
                if isinstance(bench_dd_tuple, tuple) and len(bench_dd_tuple) > 2
                else None
            )
            bench_dd_duration = (
                bench_dd_tuple[3]
                if isinstance(bench_dd_tuple, tuple) and len(bench_dd_tuple) > 3
                else None
            )
            ...
            if bench_top_dds:
                bench_avg_dd_duration = sum(
                    dd["duration_days"] for dd in bench_top_dds
                ) / len(bench_top_dds)
                bench_recoveries = [
                    dd["recovery_days"] for dd in bench_top_dds if dd["recovery_days"]
                ]
                if bench_recoveries:
            ...
            bench_metrics = {
                ...
                "ulcer": bench_ulcer,
                "pain": bench_pain,
                "var_90": calculate_var(benchmark_returns, 0.90, method="historical")
                or 0,
                "var_95": calculate_var(benchmark_returns, 0.95, method="historical")
                or 0,
                "var_99": calculate_var(benchmark_returns, 0.99, method="historical")
                or 0,
                "var_95_param": calculate_var(
                    benchmark_returns, 0.95, method="parametric"
                )
                or 0,
                "var_95_cf": calculate_var(
                    benchmark_returns, 0.95, method="cornish_fisher"
                )
                or 0,
    ...
    metrics_data = [
        (
            ...
            (
                f"{(p_daily - bench_metrics.get('daily_vol', 0))*100:+.2f}%"
                if bench_metrics
                else "—"
            ),
        ),
        (
            ...
            (
                f"{(p_weekly - bench_metrics.get('weekly_vol', 0))*100:+.2f}%"
                if bench_metrics
                else "—"
            ),
        ),
        (
            ...
            (
                f"{(p_monthly - bench_metrics.get('monthly_vol', 0))*100:+.2f}%"
                if bench_metrics
                else "—"
            ),
        ),
        (
            ...
            (
                f"{(p_annual - bench_metrics.get('annual_vol', 0))*100:+.2f}%"
                if bench_metrics
                else "—"
            ),
        ),
        (
            "Max Drawdown",
            f"{port_dd_val*100:.2f}%",
            f"{bench_metrics.get('max_dd', 0)*100:.2f}%" if bench_metrics else "—",
            (
                f"{(port_dd_val - bench_metrics.get('max_dd', 0))*100:+.2f}%"
                if bench_metrics
                else "—"
            ),
        ),
        (
            "Max DD Peak Date",
            str(port_dd_date)[:10] if port_dd_date else "—",
            (
                str(bench_metrics.get("max_dd_date"))[:10]
                if bench_metrics and bench_metrics.get("max_dd_date")
                else "—"
            ),
            "—",
        ),
        (
            "Max DD Trough Date",
            str(port_dd_trough)[:10] if port_dd_trough else "—",
            (
                str(bench_metrics.get("max_dd_trough"))[:10]
                if bench_metrics and bench_metrics.get("max_dd_trough")
                else "—"
            ),
            "—",
        ),
        (
            ...
            str(port_dd_duration) if port_dd_duration else "—",
            (
                str(bench_metrics.get("max_dd_duration"))
                if bench_metrics and bench_metrics.get("max_dd_duration")
                else "—"
            ),
            (
                f"{int(port_dd_duration or 0) - int(bench_metrics.get('max_dd_duration') or 0):+d}"
                if bench_metrics
                and port_dd_duration
                and bench_metrics.get("max_dd_duration")
                else "—"
            ),
        ),
        (
            "Current Drawdown",
            f"{port_current_dd*100:.2f}%",
            f"{bench_metrics.get('current_dd', 0)*100:.2f}%" if bench_metrics else "—",
            (
                f"{(port_current_dd - bench_metrics.get('current_dd', 0))*100:+.2f}%"
                if bench_metrics and port_current_dd != 0
                else "—"
            ),
        ),
        (
            "Average Drawdown",
            f"{port_avg_dd*100:.2f}%",
            f"{bench_metrics.get('avg_dd', 0)*100:.2f}%" if bench_metrics else "—",
            (
                f"{(port_avg_dd - bench_metrics.get('avg_dd', 0))*100:+.2f}%"
                if bench_metrics
                else "—"
            ),
        ),
        (
            "Avg DD Duration",
            f"{int(avg_dd_duration)} days" if avg_dd_duration else "—",
            (
                f"{int(bench_metrics.get('avg_dd_duration', 0))} days"
                if bench_metrics and bench_metrics.get("avg_dd_duration")
                else "—"
            ),
            (
                f"{int(avg_dd_duration or 0) - int(bench_metrics.get('avg_dd_duration') or 0):+d} days"
                if bench_metrics
                and avg_dd_duration
                and bench_metrics.get("avg_dd_duration")
                else "—"
            ),
        ),
        (
            "Avg Recovery Time",
            f"{int(avg_recovery_time)} days" if avg_recovery_time else "—",
            (
                f"{int(bench_metrics.get('avg_recovery', 0))} days"
                if bench_metrics and bench_metrics.get("avg_recovery")
                else "—"
            ),
            (
                f"{int(avg_recovery_time or 0) - int(bench_metrics.get('avg_recovery') or 0):+d} days"
                if bench_metrics
                and avg_recovery_time
                and bench_metrics.get("avg_recovery")
                else "—"
            ),
        ),
        (
            "Max Recovery Time",
            f"{int(max_recovery_time)} days" if max_recovery_time else "—",
            (
                f"{int(bench_metrics.get('max_recovery', 0))} days"
                if bench_metrics and bench_metrics.get("max_recovery")
                else "—"
            ),
            (
                f"{int(max_recovery_time or 0) - int(bench_metrics.get('max_recovery') or 0):+d} days"
                if bench_metrics
                and max_recovery_time
                and bench_metrics.get("max_recovery")
                else "—"
            ),
        ),
        (
            ...
            (
                f"{(port_ulcer - bench_metrics.get('ulcer', 0))*100:+.2f}%"
                if bench_metrics
                else "—"
            ),
        ),
        (
            ...
            (
                f"{(port_pain - bench_metrics.get('pain', 0))*100:+.2f}%"
                if bench_metrics
                else "—"
            ),
        ),
        (
            "VaR 90% (Historical)",
            f"{port_var_90*100:.2f}%",
            f"{bench_metrics.get('var_90', 0)*100:.2f}%" if bench_metrics else "—",
            (
                f"{(port_var_90 - bench_metrics.get('var_90', 0))*100:+.2f}%"
                if bench_metrics
                else "—"
            ),
        ),
        (
            ...
            (
                f"{(port_var_95 - bench_metrics.get('var_95', 0))*100:+.2f}%"
                if bench_metrics
                else "—"
            ),
        ),
        (
            "VaR 99% (Historical)",
            f"{port_var_99*100:.2f}%",
            f"{bench_metrics.get('var_99', 0)*100:.2f}%" if bench_metrics else "—",
            (
                f"{(port_var_99 - bench_metrics.get('var_99', 0))*100:+.2f}%"
                if bench_metrics
                else "—"
            ),
        ),
        (
            "VaR 95% (Parametric)",
            f"{port_var_95_param*100:.2f}%",
            (
                f"{bench_metrics.get('var_95_param', 0)*100:.2f}%"
                if bench_metrics
                else "—"
            ),
            (
                f"{(port_var_95_param - bench_metrics.get('var_95_param', 0))*100:+.2f}%"
                if bench_metrics
                else "—"
            ),
        ),
        (
            ...
            f"{port_var_95_cf*100:.2f}%",
            f"{bench_metrics.get('var_95_cf', 0)*100:.2f}%" if bench_metrics else "—",
            (
                f"{(port_var_95_cf - bench_metrics.get('var_95_cf', 0))*100:+.2f}%"
                if bench_metrics
                else "—"
            ),
        ),
        (
            "CVaR 90%",
            f"{port_cvar_90*100:.2f}%",
            f"{bench_metrics.get('cvar_90', 0)*100:.2f}%" if bench_metrics else "—",
            (
                f"{(port_cvar_90 - bench_metrics.get('cvar_90', 0))*100:+.2f}%"
                if bench_metrics
                else "—"
            ),
        ),
        (
            ...
            (
                f"{(port_cvar_95 - bench_metrics.get('cvar_95', 0))*100:+.2f}%"
                if bench_metrics
                else "—"
            ),
        ),
        (
            "CVaR 99%",
            f"{port_cvar_99*100:.2f}%",
            f"{bench_metrics.get('cvar_99', 0)*100:.2f}%" if bench_metrics else "—",
            (
                f"{(port_cvar_99 - bench_metrics.get('cvar_99', 0))*100:+.2f}%"
                if bench_metrics
                else "—"
            ),
        ),
        (
            "Downside Deviation",
            f"{port_downside*100:.2f}%",
            f"{bench_metrics.get('downside', 0)*100:.2f}%" if bench_metrics else "—",
            (
                f"{(port_downside - bench_metrics.get('downside', 0))*100:+.2f}%"
                if bench_metrics
                else "—"
            ),
        ),
        (
            ...
            (
                f"{(port_semi - bench_metrics.get('semi', 0))*100:+.2f}%"
                if bench_metrics
                else "—"
            ),
        ),
        (
            ...
            (
                f"{(port_skew - bench_metrics.get('skew', 0)):+.3f}"
                if bench_metrics
                else "—"
            ),
        ),
        (
            ...
            (
                f"{(port_kurt - bench_metrics.get('kurt', 0)):+.3f}"
                if bench_metrics
                else "—"
            ),
        ),
    ...
    complete_risk_df = pd.DataFrame(
        metrics_data, columns=["Metric", "Portfolio", "Benchmark", "Difference"]
    )
    ...
    st.dataframe(
        complete_risk_df, use_container_width=True, hide_index=True, height=total_height
    )
...
def _render_drawdown_analysis(
    risk, portfolio_values, benchmark_values, portfolio_returns, benchmark_returns
):
    ...
    if drawdown_periods_data:
        ...
        if periods:
            ...
            avg_duration = sum(p.get("duration_days", 0) for p in periods) / len(
                periods
            )
            interpretation = _interpret_drawdown_periods(
                periods, avg_depth, avg_duration
            )
            st.info(interpretation)
                ...
                with st.expander(
                    ...
                    with col4:
                        total_days = dd["duration_days"] + (
                            dd["recovery_days"] if dd["recovery_days"] else 0
                        )
                    ...
                    st.plotly_chart(
                        fig, use_container_width=True, key=f"recovery_{dd['number']}"
                    )
                    ...
                    if dd["peak_value"] and dd["trough_value"]:
                        st.caption(
                            ...
                            + (
                                f" → Recovery: {dd['recovery_value']:.2f}"
                                if dd["recovery_value"]
                                else " → Not recovered"
                            )
                        )
    ...
    if portfolio_returns is not None and not portfolio_returns.empty:
        ...
        if top_drawdowns:
                ...
                dd_data.append(
                    {
                        "#": i,
                        "Start (Peak)": (
                            dd["start_date"].strftime("%Y-%m-%d")
                            if dd["start_date"]
                            else "—"
                        ),
                        "Bottom (Trough)": (
                            dd["bottom_date"].strftime("%Y-%m-%d")
                            if dd["bottom_date"]
                            else "—"
                        ),
                        "Recovery (End)": (
                            dd["recovery_date"].strftime("%Y-%m-%d")
                            if dd["recovery_date"]
                            else "Ongoing"
                        ),
                        ...
                        "Recovery (days)": (
                            dd["recovery_days"] if dd["recovery_days"] else "—"
                        ),
                    }
                )
            ...
            longest_recovery = max(
                (dd["recovery_days"] for dd in top_drawdowns if dd["recovery_days"]),
                default=None,
            )
            recovery_text = f"{longest_recovery} days" if longest_recovery else "N/A"
            st.caption(
                f"**Summary:** Worst drawdown: {worst_depth*100:.2f}% | Longest duration: {longest_duration} days | Longest recovery: {recovery_text}"
            )
            ...
            # Portfolio metrics
            avg_dd_depth = (
                sum(dd["depth"] for dd in top_drawdowns) / len(top_drawdowns)
                if top_drawdowns
                else 0
            )
            avg_dd_duration = (
                sum(dd["duration_days"] for dd in top_drawdowns) / len(top_drawdowns)
                if top_drawdowns
                else 0
            )
            avg_recovery = (
                sum(dd["recovery_days"] for dd in top_drawdowns if dd["recovery_days"])
                / len([dd for dd in top_drawdowns if dd["recovery_days"]])
                if any(dd["recovery_days"] for dd in top_drawdowns)
                else None
            )
            max_recovery = max(
                (dd["recovery_days"] for dd in top_drawdowns if dd["recovery_days"]),
            ...
            if benchmark_returns is not None and not benchmark_returns.empty:
                try:
                    bench_drawdowns = calculate_top_drawdowns(
                        benchmark_returns, top_n=5
                    )

                    if bench_drawdowns:
                        bench_avg_dd_depth = sum(
                            dd["depth"] for dd in bench_drawdowns
                        ) / len(bench_drawdowns)
                        bench_avg_dd_duration = sum(
                            dd["duration_days"] for dd in bench_drawdowns
                        ) / len(bench_drawdowns)
                        bench_avg_recovery = (
                            sum(
                                dd["recovery_days"]
                                for dd in bench_drawdowns
                                if dd["recovery_days"]
                            )
                            / len([dd for dd in bench_drawdowns if dd["recovery_days"]])
                            if any(dd["recovery_days"] for dd in bench_drawdowns)
                            else None
                        )
                        bench_max_recovery = max(
                            (
                                dd["recovery_days"]
                                for dd in bench_drawdowns
                                if dd["recovery_days"]
                        ...
                        comparison_data[0][
                            "Benchmark"
                        ] = f"{bench_avg_dd_depth*100:.2f}%"
                        comparison_data[1][
                            "Benchmark"
                        ] = f"{bench_avg_dd_duration:.0f} days"
                        comparison_data[2]["Benchmark"] = (
                            f"{bench_avg_recovery:.0f} days"
                            if bench_avg_recovery
                            else "—"
                        )
                        comparison_data[3]["Benchmark"] = (
                            f"{bench_max_recovery:.0f} days"
                            if bench_max_recovery
                            else "—"
                        )
                except Exception as e:
            ...
            if benchmark_returns is not None and not benchmark_returns.empty:
                try:
                    ...
                    if bench_drawdowns:
                        ...
                        parts = []
                        parts.append("**Benchmark Comparison Analysis:**")
                        parts.append(
                            f"Average drawdown depth: Portfolio {avg_dd_depth*100:.2f}% vs Benchmark {bench_avg_dd_depth*100:.2f}%"
                        )

                        if abs(avg_dd_depth - bench_avg_dd_depth) < 0.01:
                            parts.append("Similar drawdown depths")
                        elif avg_dd_depth < bench_avg_dd_depth:
                            parts.append("Portfolio shows deeper drawdowns")
                        else:
                            parts.append("Portfolio shows shallower drawdowns")

                        if avg_recovery and bench_avg_recovery:
                            parts.append(
                                f"Average recovery time: Portfolio {avg_recovery:.0f} days vs Benchmark {bench_avg_recovery:.0f} days"
                            )
                            if abs(avg_recovery - bench_avg_recovery) < 10:
                                parts.append("Similar recovery times")
                            elif avg_recovery < bench_avg_recovery:
                                parts.append("Portfolio recovers faster")
                            else:
                                parts.append("Portfolio recovers slower")
...
def _render_var_analysis(portfolio_returns, benchmark_returns):
    ...
    var_cf = (
        calculate_var(portfolio_returns, conf_decimal, method="cornish_fisher") or 0
    )
    ...
    if benchmark_returns is not None and not benchmark_returns.empty:
        try:
            bench_var_hist = (
                calculate_var(benchmark_returns, conf_decimal, method="historical") or 0
            )
    ...
    var_comparison_data = [
        (
            "Historical",
            f"{var_hist*100:.2f}%",
            f"{int((1-conf_decimal)*100)}% of days worse",
        ),
    ...
    try:
        fig = plot_var_distribution(portfolio_returns, var_hist, cvar, conf_decimal)
        st.plotly_chart(fig, use_container_width=True, key="var_distribution")

        # Add interpretation
        interpretation = _interpret_var_distribution(
            var_hist, cvar, conf_decimal, var_param, var_cf
        )
        st.info(interpretation)
    except Exception as e:
...
def _render_rolling_risk(portfolio_returns, benchmark_returns, risk_free_rate):
    ...
    try:
        vol_data = get_rolling_volatility_data(
            portfolio_returns, benchmark_returns, window=window_size
        )
        if vol_data:
            fig = plot_rolling_volatility(vol_data)
            st.plotly_chart(
                fig, use_container_width=True, key="rolling_volatility_chart"
            )

            # Automatic interpretation
            if vol_data and "portfolio" in vol_data:
                port_vol = vol_data["portfolio"]
                interpretation = _interpret_rolling_metric(
                    port_vol, "Volatility", "lower", window_size
                )
                st.info(interpretation)
            ...
            metrics_data = [
                {
                    ...
                    "benchmark_value": (
                        bench_stats.get("avg", 0) if bench_stats else None
                    ),
                    "format": "percent",
                    "higher_is_better": False,
                },
                {
                    ...
                    "benchmark_value": (
                        bench_stats.get("median", 0) if bench_stats else None
                    ),
                    "format": "percent",
                    "higher_is_better": False,
                },
                {
                    ...
                    "benchmark_value": (
                        bench_stats.get("min", 0) if bench_stats else None
                    ),
                    "format": "percent",
                    "higher_is_better": False,
                },
                {
                    ...
                    "benchmark_value": (
                        bench_stats.get("max", 0) if bench_stats else None
                    ),
                    "format": "percent",
                    "higher_is_better": False,
                },
    ...
    try:
        ...
        if sharpe_data:
            ...
            if sharpe_data and "portfolio" in sharpe_data:
                ...
                interpretation = _interpret_rolling_metric(
                    port_sharpe, "Sharpe Ratio", "higher", window_size, threshold=1.0
                )
                st.info(interpretation)
    except Exception as e:
    ...
    try:
        ...
        if sortino_data:
            ...
            if sortino_data and "portfolio" in sortino_data:
                ...
                interpretation = _interpret_rolling_metric(
                    port_sortino, "Sortino Ratio", "higher", window_size, threshold=1.0
                )
                st.info(interpretation)
    except Exception as e:
    ...
    if benchmark_returns is not None and not benchmark_returns.empty:
        ...
        try:
            ...
            if alpha_data:
                ...
                st.plotly_chart(
                    fig, use_container_width=True, key="rolling_alpha_chart"
                )
                ...
                if alpha_data and "alpha" in alpha_data:
                    ...
                    interpretation = _interpret_rolling_metric(
                        port_alpha, "Alpha", "higher", window_size, threshold=0.0
                    )
                    if interpretation:
                        st.info(interpretation)
        except Exception as e:
        ...
        try:
            ...
            if active_return_data:
                ...
                st.plotly_chart(
                    fig, use_container_width=True, key="rolling_active_return_chart"
                )
                ...
                col2.metric(
                    "Periods with Positive Alpha",
                    f"{stats.get('pct_positive', 0):.1f}%",
                )
        ...
        try:
            ...
            if bull_bear_data:
                ...
                # Statistics Table
                st.markdown("**Performance in Different Market Conditions**")
                st.caption(
                ...
                st.caption("Median daily returns in bullish vs bearish market days")
                fig_comparison = plot_bull_bear_returns_comparison(bull_bear_data)
                st.plotly_chart(
                    fig_comparison, use_container_width=True, key="bull_bear_comparison"
                )
                ...
                st.plotly_chart(
                    fig_rolling, use_container_width=True, key="bull_bear_rolling_beta"
                )
        ...
        st.info(
            "Benchmark data required for Beta, Alpha, Active Return, and Bull/Bear analysis"
        )


def _render_assets_tab(
    positions, portfolio_returns, benchmark_returns, portfolio_id, portfolio_service
):
    """Render Assets & Correlations tab with sub-tabs."""
    sub_tab1, sub_tab2, sub_tab3 = st.tabs(
        ["Asset Overview & Impact", "Correlations", "Asset Details & Dynamics"]
    )
...
def _render_asset_overview(positions):
    ...
    st.caption(
        "**Change%** shows daily price change " "(today vs previous trading day)"
    )
        ...
        if portfolio_returns is not None and not portfolio_returns.empty:
            ...
            for ticker in tickers:
                try:
                    ...
                    logger.warning(f"Failed to fetch full prices for {ticker}: {e}")
    ...
    try:
        ...
        if price_data_full is None or price_data_full.empty:
            ...
            if impact_return_data and impact_return_data.get("tickers"):
                ...
                st.plotly_chart(fig, use_container_width=True, key="impact_on_return")
                ...
                st.info(
                    "Insufficient data for impact on return analysis. "
    ...
    try:
        if (
            price_data_full is not None
            and not price_data_full.empty
    ...
    try:
        if price_data_full is not None and not price_data_full.empty:
            ...
            if div_data:
                ...
                st.markdown("")
                st.markdown(
                    "**Formula:** Weighted sum of volatilities / Portfolio volatility"
                )
                st.markdown("")
                ...
                if is_diversified:
                    ...
                    st.warning(
                        "Value <= 1.0 indicates little to no diversification benefit\n\n"
                        "Portfolio may be under-diversified or concentrated"
                    )
                ...
                if len(factor_data) < 3:
                    ...
                    for ticker in etf_tickers:
                        try:
                            ...
                            if not prices.empty:
                                returns = (
                                    prices.set_index("Date")["Adjusted_Close"]
                                    .pct_change()
                                    .dropna()
                                )
            ...
            if len(factor_data) >= 1:
                ...
                if len(aligned_data) > 20:
                    ...
                    for i in range(1, len(beta)):
                        contrib = (
                            (beta[i] * X_factors.iloc[:, i - 1].values).var()
                            / explained_var
                            * 100
                        )
                    ...
                    if total_contrib > 0:
                        factor_contributions = [
                            c / total_contrib * 100 for c in factor_contributions
                        ]
                    ...
                    results = sorted(
                        results, key=lambda x: abs(x["Contribution"]), reverse=True
                    )
                    ...
                    for r in chart_data:
                        ...
                        if abs_contrib > 0.01:  # Threshold: 0.01%
                            ...
                            text_info.append(f"{sign}{abs_contrib:.1f}%")
                    ...
                    st.plotly_chart(
                        fig, use_container_width=True, key="factor_contribution"
                    )
                    ...
                    with col3:
                        ...
                        st.metric("Alpha (Annual)", f"{alpha_annual:.2f}%")
                    ...
                    dominant_factors = [
                        r for r in results if abs(r["Contribution"]) > 10.0
                    ]
                    ...
                    top_2_contrib = sum([abs(r["Contribution"]) for r in results[:2]])
                    if top_2_contrib > 80:
    ...
    try:
        ...
        if corr_matrix_data and corr_matrix_data.get("correlation_matrix") is not None:
            ...
            if corr_stats:
                ...
                with col1:
                    st.metric(
                        "Average Correlation",
                        f"{corr_stats['average_correlation']:.2f}",
                    )
                with col2:
                    st.metric(
                        "Median Correlation", f"{corr_stats['median_correlation']:.2f}"
                    )
                ...
                if avg_corr < 0.3:
                    ...
                    st.warning(
                        "⚠ High average correlation - Limited " "diversification"
                    )
        ...
        if benchmark_returns is not None and not benchmark_returns.empty:
            ...
            if corr_bench_data and corr_bench_data.get("tickers"):
                ...
                st.plotly_chart(
                    fig, use_container_width=True, key="corr_with_benchmark_chart"
                )
            ...
            if avg_corr_data:
                ...
                fig_avg.add_trace(
                    go.Bar(
                        ...
                        text=[
                            f"{c*100:.1f}%" for c in avg_corr_data["avg_correlations"]
                        ],
                        textposition="outside",
                    )
                )
                ...
                avg_corr_df = pd.DataFrame(
                    {
                        ...
                        "Avg Correlation": [
                            f"{c:.3f}" for c in avg_corr_data["avg_correlations"]
                        ],
                        "Diversification Score": [
                            f"{d:.3f}" for d in avg_corr_data["diversification_scores"]
                        ],
                    }
                )
                ...
                # Automatic interpretation
                interpretation = _interpret_average_correlation_to_portfolio(
                    avg_corr_data
                )
                if interpretation:
                    st.info(interpretation)
        ...
        if price_data is not None:
            ...
            if (
                corr_matrix_data
                and corr_matrix_data.get("correlation_matrix") is not None
            ):
                ...
                if rolling_corr_data:
                    ...
                    for pair_name, corr_series in rolling_corr_data[
                    ...
                    fig_rolling.update_layout(
                        title=(
                            f"Rolling Correlations " f"({rolling_window_corr} days)"
                        ),
                        xaxis_title="Date",
                    ...
                    # Automatic interpretation
                    interpretation = _interpret_rolling_correlations(
                        rolling_corr_data, rolling_window_corr
                    )
                    if interpretation:
                        st.info(interpretation)
    ...
    try:
        ...
        if benchmark_returns is not None and not benchmark_returns.empty:
            ...
            if rolling_corr_data and rolling_corr_data.get("rolling_correlations"):
                ...
                # Automatic interpretation
                interpretation = _interpret_rolling_correlation_with_benchmark(
                    rolling_corr_data, window
                )
                if interpretation:
                    st.info(interpretation)
            else:
        ...
        if selected_ticker:
            ...
            if detailed_data:
                ...
                if benchmark_returns is not None and not benchmark_returns.empty:
                    try:
                        ...
                        if len(aligned_bench) >= 2:
                            ...
                            bench_volatility = calculate_volatility(aligned_bench).get(
                                "annual", 0.0
                            )
                            bench_sharpe = (
                                calculate_sharpe_ratio(
                ...
                detailed_data["portfolio_metrics"]
                ...
                # Automatic interpretation
                interpretation = _interpret_price_volume_chart(
                    detailed_data, selected_ticker
                )
                if interpretation:
                    st.info(interpretation)
                ...
                st.markdown("---")
                st.subheader(
                    f"Comparison of Return - {selected_ticker} vs Portfolio vs SPY"
                )
                ...
                if len(common_dates) >= 2:
                    ...
                    fig.update_layout(**layout)
                    st.plotly_chart(
                        fig, use_container_width=True, key="asset_cumulative"
                    )

                    # Automatic interpretation
                    interpretation = _interpret_comparison_of_return(
                        detailed_data, selected_ticker
                    )
                    if interpretation:
                        st.info(interpretation)
                ...
                if other_corrs:
                    ...
                    st.plotly_chart(
                        fig, use_container_width=True, key="asset_correlations"
                    )

                    # Automatic interpretation
                    interpretation = _interpret_asset_correlations(
                        other_corrs, selected_ticker
                    )
                    if interpretation:
                        st.info(interpretation)
                else:
    ...
    with col1:
        ...
        tabs_config = {
            ...
            "Performance": st.checkbox(
                "Performance", value=True, key="pdf_performance"
            ),
            "Risk": st.checkbox("Risk", value=True, key="pdf_risk"),
            "Assets & Correlations": st.checkbox(
                "Assets & Correlations", value=False, key="pdf_assets"
            ),
        }
    ...
    if st.button("📥 Generate PDF Report", type="primary", use_container_width=True):
        if not portfolio_returns.empty:
            ...
            if total_pages == 0:
                ...
                with st.spinner(
                    f"Generating PDF report from {total_pages} tab(s)... This may take a minute."
                ):
                    try:
                        ...
                        if success and os.path.exists(pdf_path):
                            ...
                            # Download button
                            st.success(
                                f"✅ PDF generated successfully! ({file_size:.2f} MB, {total_pages} pages)"
                            )
                            st.download_button(
                            ...
                            st.error(
                                "❌ Failed to generate PDF. Please check the logs for details."
                            )
            ...
            st.warning(
                "⚠️ No portfolio returns data available. Please calculate metrics first."
            )
