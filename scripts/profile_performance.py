...
def profile_portfolio_creation(iterations: int = 10) -> dict[str, float]:
    ...
    print("  Target: <100ms | Acceptable: <500ms")
...
def profile_data_fetching_cached(iterations: int = 10) -> dict[str, float]:
    ...
    print(f"  Min: {min_time*1000:.2f}ms")
    print(f"  Max: {max_time*1000:.2f}ms")
    print("  Target: <10ms | Acceptable: <50ms")
...
def profile_metrics_calculation(iterations: int = 5) -> dict[str, float]:
    ...
    try:
        ...
        test_portfolio = next(
            (p for p in portfolios if p.name == "Performance Test Portfolio"), None
        )
        if test_portfolio:
    ...
    print(f"  Min: {min_time*1000:.2f}ms")
    print(f"  Max: {max_time*1000:.2f}ms")
    print("  Target: <500ms | Acceptable: <1000ms")
...
def profile_analytics_engine_directly() -> dict[str, float]:
    ...
    np.random.seed(42)
    dates = pd.date_range(
        start=date.today() - timedelta(days=365), periods=252, freq="D"
    )
